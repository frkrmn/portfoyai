...
import { structuredLog, trackAiCall } from "./observability.mjs";
...
export async function backfillSiteContent({ siteId, userId, supabase = getSupabaseClient(), generate, now = () => new Date(), authorized = false }) {
  ...
  if (attempts >= contentBackfillDailyQuota) {
    ...
    structuredLog("warn", "ai.backfill.quota_exceeded", { site_id: site.id, attempts, retry_after_seconds: retryAfter });
  ...
  try {
    const generated = generate ? await generate({ source, responseSchema }) : await trackAiCall({
      operation: "site.translate_content",
      model: siteConfigModel,
      call: () => new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY }).models.generateContent({
        model: siteConfigModel,
    ...
    structuredLog("info", "ai.backfill.completed", { site_id: site.id, attempt: translation.attempts });
    ...
    structuredLog("error", "ai.backfill.failed", { site_id: site.id, attempt: translation.attempts, error });
    throw error;
  }
}
