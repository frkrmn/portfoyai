...
export default async function handler(request, response) {
  ...
  try {
    ...
    const { data, error } = await getSupabaseClient()
      .from("sites")
      .select("id, slug, business_name, tone, primary_color, accent_color, headline, theme_config, previous_theme_config, status, country_id, province_id, district_id, neighborhood_id, created_at")
      .eq("user_id", user.id)
