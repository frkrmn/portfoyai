...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (siteId) { const owned = await getSupabaseClient().from("sites").select("id").eq("id", siteId).eq("user_id", user.id).maybeSingle(); if (owned.error) throw owned.error; if (!owned.data) return sendJson(response, 404, { error: "Site not found." }); }
