...
const payload = (slug) => {
  ...
  return {
    ...
    config: { template_id: templateId, business_name: "Bilingual Realty", tone: "Agent-authored content stays intact.", primary_color: "#173f32", accent_color: "#d7a84b", headline: "Agent-authored headline", theme_config: { template_id: templateId, content: templateId === "guided-match" ? legacyContent : {} } },
...
socket.addEventListener("message", (event) => {
  ...
  if (message.method === "Fetch.requestPaused") {
    ...
    if (url.pathname.endsWith("/content-backfill")) {
      ...
      void command("Fetch.fulfillRequest", { requestId: message.params.requestId, responseCode: 404, responseHeaders: [{ name: "Content-Type", value: "application/json" }], body: Buffer.from(JSON.stringify({ error: "Not found" })).toString("base64") });
      return;
    }
...
try {
  ...
  await waitFor("document.body.innerText.includes('Doğru ev, doğru sorularla bulunur')");
  assert.equal(legacyBackfillCalls, 0);
  ...
  console.info(JSON.stringify({ templates_tested: ["land-plots", "investment-focused", "guided-match"], default_language: true, immediate_toggle: true, header_nav_buttons_badges_filters_forms: true, anonymous_backfill_calls: legacyBackfillCalls, public_content_uses_safe_fallback: true, session_override_after_reload: true }, null, 2));
} finally {
  socket.close();
}
