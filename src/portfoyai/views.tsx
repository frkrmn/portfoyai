      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <section className="space-y-3 rounded-2xl border bg-white p-4 md:col-span-2">
            <div className="font-semibold">{t("dashboard.seo.listingTitle")}</div>
            <div className="grid gap-3 md:grid-cols-2"><div><Label>{t("dashboard.seo.titleTr")}</Label><Input maxLength={70} value={draft.seo?.title?.tr || ""} onChange={(event) => onDraftChange({ seo: { ...draft.seo, title: { ...draft.seo?.title, tr: event.target.value } } })} /></div><div><Label>{t("dashboard.seo.titleEn")}</Label><Input maxLength={70} value={draft.seo?.title?.en || ""} onChange={(event) => onDraftChange({ seo: { ...draft.seo, title: { ...draft.seo?.title, en: event.target.value } } })} /></div></div>
            <div className="grid gap-3 md:grid-cols-2"><div><Label>{t("dashboard.seo.descriptionTr")}</Label><Textarea maxLength={170} value={draft.seo?.description?.tr || ""} onChange={(event) => onDraftChange({ seo: { ...draft.seo, description: { ...draft.seo?.description, tr: event.target.value } } })} /></div><div><Label>{t("dashboard.seo.descriptionEn")}</Label><Textarea maxLength={170} value={draft.seo?.description?.en || ""} onChange={(event) => onDraftChange({ seo: { ...draft.seo, description: { ...draft.seo?.description, en: event.target.value } } })} /></div></div>
            <div><Label>{t("dashboard.seo.ogImage")}</Label><Input type="url" value={draft.seo?.og_image || ""} onChange={(event) => onDraftChange({ seo: { ...draft.seo, og_image: event.target.value } })} /></div>
            <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={draft.seo?.robots_index !== false} onChange={(event) => onDraftChange({ seo: { ...draft.seo, robots_index: event.target.checked } })} />{t("dashboard.seo.index")}</label>
          </section>
        </div>
