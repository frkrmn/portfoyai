...
const getThemeStyles = (
  theme: Pick<ThemeConfig, "primary" | "accent" | "fontPairing">,
) =>
  ({
  ...
  const navigation = [
    {
      ...
      label: t("dashboard.shell.overview"),
    },
    {
      id: "analytics" as const,
      icon: BarChart3,
      label: t("dashboard.shell.analytics"),
    },
    {
      id: "listings" as const,
      icon: Home,
      label: t("dashboard.shell.listings"),
    },
    {
      id: "content" as const,
      icon: FileText,
      label: t("dashboard.shell.content"),
    },
    ...
    { id: "leads" as const, icon: Users, label: t("dashboard.shell.leads") },
    {
      id: "site" as const,
      icon: Palette,
      label: t("dashboard.shell.siteSettings"),
    },
  ];
    ...
    <div className="min-h-screen bg-[#f2efe8] text-[#17231e]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[260px] flex-col bg-[#173f32] px-5 py-6 text-white lg:flex">
        <Link to="/" className="flex items-center gap-3 px-2">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-white text-[#173f32]">
        ...
        <div className="mt-10 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/35">
        ...
        <nav className="mt-3 space-y-1.5">
          {navigation.map(({ id, icon: Icon, label }) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white",
                activeSection === id
                  ? "bg-white text-[#173f32] shadow-sm"
            ...
            >
              ...
              {id === "leads" && leadCount > 0 ? (
                <span className="ml-auto rounded-full bg-[#d86f45] px-2 py-0.5 text-[10px] text-white">
                  {leadCount}
                </span>
              ) : null}
            </button>
          ...
          {isAdmin ? (
            <Link
              ...
              className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-white/65 hover:bg-white/8 hover:text-white"
            >
              <FileText className="h-[18px] w-[18px]" />
              <span>Platform Landing CMS</span>
            </Link>
          ) : null}
        </nav>
        <div className="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-[#dbe5d2] font-semibold text-[#173f32]">
              {identity
                .split(" ")
                .map((part) => part[0])
                .join("")
        ...
        <header className="sticky top-0 z-20 border-b border-[#173f32]/10 bg-[#f2efe8]/90 px-4 py-4 backdrop-blur-xl sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
            ...
            <div className="hidden lg:block">
              <div className="text-xs text-[#78827c]">
                {businessName || t("dashboard.shell.noSite")}
              </div>
              <div className="mt-0.5 text-sm font-semibold">
                {t("dashboard.shell.adminPanel")}
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              {user ? (
                <Button
                  variant="outline"
                  size="icon"
                  title={t("dashboard.shell.logout")}
                  onClick={() =>
                    void signOut().catch((error) => toast.error(error.message))
                  }
                  className="rounded-full border-[#173f32]/10 bg-white text-[#173f32]"
                >
        ...
        {mobileMenuOpen ? (
          <div className="fixed inset-0 z-40 lg:hidden">
            ...
            <aside
              ...
              role="dialog"
              aria-modal="true"
              aria-label={t("dashboard.shell.mobileNavigation")}
              className="absolute inset-y-0 left-0 flex w-[min(86vw,340px)] flex-col bg-[#173f32] p-5 text-white shadow-2xl"
            >
              ...
              <nav className="mt-8 space-y-1.5">
                {navigation.map(({ id, icon: Icon, label }, index) => (
                  <button
                    ref={index === 0 ? firstMobileItem : undefined}
                    type="button"
                    key={id}
                    aria-current={activeSection === id ? "page" : undefined}
                    onClick={() => selectSection(id)}
                ...
                {isAdmin ? (
                  <Link
                    to="/admin/landing-content"
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-white/70"
                  >
                    <FileText className="h-[18px] w-[18px]" />
              ...
              <div className="mt-auto text-sm text-white/60">{identity}</div>
            </aside>
          </div>
        ) : null}
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        toast.success(
          "Talep kaydedildi. Bildirim kuyruğu stub olarak loglandı.",
        );
    ...
    >
      ...
      <Button
        type="submit"
        className="w-full gap-2 bg-[linear-gradient(135deg,var(--theme-primary),var(--theme-accent))] text-white hover:opacity-95"
      >
    ...
    return (
      ...
      >
        ...
        <Button
          ...
          className="mt-4 rounded-full border-emerald-300 bg-white text-emerald-900"
        >
          Yeni mesaj gönder
        </Button>
      </div>
    ...
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          setSubmitError(
            error instanceof Error ? error.message : "Talebiniz gönderilemedi.",
          );
        } finally {
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Aradığınız evi veya ilgilendiğiniz portföyü kısaca anlatabilirsiniz."
        />
      </div>
      {submitError ? (
        <p
      ...
      <Button
        ...
        className="w-full gap-2 rounded-full bg-[#173f32] text-white"
      >
        <Send className="h-4 w-4" />
...
export function ListingForm({
  ...
  const [socialKitUrls, setSocialKitUrls] = useState<{
    post: string;
    story: string;
  } | null>(null);
  ...
  const copyFactsSignature = JSON.stringify([
    ...
    draft.category,
    draft.bedroom_count,
    draft.bathroom_count,
    draft.rental_yield_percent,
    draft.roi_notes,
    draft.urgent_sale,
  ...
  useEffect(() => {
    ...
    setSocialKitUrls((current) => {
      if (current) {
        URL.revokeObjectURL(current.post);
        URL.revokeObjectURL(current.story);
      }
      return null;
    });
  }, [draft.id]);
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("dashboard.listingForm.copyError");
  ...
  const handleSocialKit = async () => {
    ...
    try {
      const [post, story] = await Promise.all([
        onLoadSocialKit("post"),
        onLoadSocialKit("story"),
      ]);
      setSocialKitUrls({
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("dashboard.listingForm.socialError");
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    const selected = Array.from(files).slice(
      0,
      Math.max(0, 10 - draft.media.length),
    );
    ...
    const next = await Promise.all(
      selected.map((file, index) =>
        uploadImage(file, siteId, "listing", draft.media.length + index),
      ),
    );
    ...
    <Card className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7] shadow-none">
      <CardHeader>
        <CardTitle className="text-2xl">
          {t(
            draft.id
              ? "dashboard.listingForm.editTitle"
              : "dashboard.listingForm.newTitle",
          )}
        </CardTitle>
        <CardDescription>
          {t("dashboard.listingForm.descriptionPlaceholder")}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2 md:col-span-2">
            ...
            <Input
              placeholder={t("dashboard.listingForm.titlePlaceholder")}
              value={draft.title}
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_110px] gap-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={draft.price}
                onChange={(e) =>
                  onDraftChange({ price: Number(e.target.value) })
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  onDraftChange({ currency: value as Listing["currency"] })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          <div className="space-y-2">
            ...
            <Input
              value={draft.m2}
              onChange={(e) => onDraftChange({ m2: Number(e.target.value) })}
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => onDraftChange({ room_count: e.target.value })}
            />
          </div>
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                onDraftChange({ listing_type: value as "sale" | "rent" })
              }
            >
              <SelectTrigger>
                <SelectValue />
              ...
              <SelectContent>
                <SelectItem value="sale">
                  {t("dashboard.listingForm.sale")}
                </SelectItem>
                <SelectItem value="rent">
                  {t("dashboard.listingForm.rent")}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select
              value={draft.property_category || "konut"}
              onValueChange={(value) =>
                onDraftChange({
                  property_category: value as Listing["property_category"],
                  property_subtype:
                    value === "konut"
                      ? "daire"
                      : value === "arsa"
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="konut">
                  {t("dashboard.listingForm.residential")}
                </SelectItem>
                <SelectItem value="arsa">
                  {t("dashboard.listingForm.land")}
                </SelectItem>
                <SelectItem value="isyeri">
                  {t("dashboard.listingForm.commercial")}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          draft.property_category === "arsa" ? (
            <div className="space-y-2" data-property-subtype-field>
              <Label>
                {t(
                  draft.property_category === "arsa"
                    ? "dashboard.listingTaxonomy.landSubtype"
                    : "dashboard.listingTaxonomy.residentialSubtype",
                )}
              </Label>
              <Select
                value={
                ...
                onValueChange={(value) =>
                  onDraftChange({
                    property_subtype: value as NonNullable<
              ...
              >
                ...
                <SelectContent>
                  {draft.property_category === "konut" ? (
                    <>
                      <SelectItem value="daire">
                        {t("dashboard.listingForm.apartment")}
                      </SelectItem>
                      <SelectItem value="mustakil_ev">
                        {t("dashboard.listingForm.detachedHouse")}
                      </SelectItem>
                      <SelectItem value="villa">
                        {t("dashboard.listingForm.villa")}
                      </SelectItem>
                      <SelectItem value="rezidans">
                        {t("dashboard.listingForm.residence")}
                      </SelectItem>
                    </>
                  ) : (
                    <>
                      <SelectItem value="konut_imarli">
                        {t("dashboard.listingTaxonomy.residentialZoned")}
                      </SelectItem>
                      <SelectItem value="ticari_imarli">
                        {t("dashboard.listingTaxonomy.commercialZoned")}
                      </SelectItem>
                      <SelectItem value="tarla_tarimsal">
                        {t("dashboard.listingTaxonomy.agriculturalField")}
                      </SelectItem>
                      <SelectItem value="villa_imarli">
                        {t("dashboard.listingTaxonomy.villaZoned")}
                      </SelectItem>
                      <SelectItem value="kentsel_donusum">
                        {t("dashboard.listingTaxonomy.urbanRenewal")}
                      </SelectItem>
                    </>
                  )}
                </SelectContent>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {draft.features.map((feature) => (
                <span
                  key={feature}
                  className="inline-flex items-center gap-2 rounded-full bg-[#173f32]/8 px-3 py-1.5 text-xs font-medium text-[#173f32]"
                >
                  {feature}
                  <button
                    type="button"
                    aria-label={`${feature} ${t("dashboard.listingForm.removePhoto")}`}
                    onClick={() =>
                      onDraftChange({
            ...
            <Input
              ...
              onKeyDown={(event) => {
                ...
                event.preventDefault();
                const feature = featureInput.trim().replace(/,$/, "");
                if (feature && !draft.features.includes(feature))
                  onDraftChange({ features: [...draft.features, feature] });
                setFeatureInput("");
              }}
              placeholder={t("dashboard.listingTaxonomy.featureHint")}
            />
          </div>
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {draft.media.map((item) => (
                <div key={item.id} className="relative">
                  ...
                  <button
                    type="button"
                    aria-label={t("dashboard.listingForm.removePhoto")}
                    onClick={() =>
                      onDraftChange({
                    ...
                    className="absolute -right-2 -top-2 grid h-6 w-6 place-items-center rounded-full bg-[#173f32] text-white"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
          ...
          <div className="space-y-2 md:col-span-2">
            <div className="flex items-center justify-between gap-3">
              <Label>{t("dashboard.listingForm.description")}</Label>
              <Button
                type="button"
                size="sm"
              ...
              >
                ...
                {t(
                  isGeneratingCopy
                    ? "dashboard.listingForm.generatingCopy"
                    : "dashboard.listingForm.generateCopy",
                )}
              </Button>
            </div>
            <Textarea
          ...
          >
            <div>
              <div className="font-semibold">
                {t("dashboard.listingForm.investmentMetrics")}
              </div>
              <p className="mt-1 text-xs text-slate-500">
                {t("dashboard.listingForm.investmentMetricsHelp")}
              </p>
            </div>
            <div className="space-y-2">
              <Label>{t("dashboard.listingForm.rentalYield")}</Label>
              <Input
                type="number"
                min="0"
                max="100"
                step="0.1"
                value={draft.rental_yield_percent ?? ""}
                onChange={(event) =>
                  onDraftChange({
            ...
            {(["rental_yield", "price_per_m2"] as const).map((key) => {
              const metric = draft.investment_metrics?.[key] || {
                source: "",
                as_of: "",
                status: "estimate" as const,
                hidden: false,
              };
              const update = (patch: Partial<typeof metric>) =>
                onDraftChange({
                  investment_metrics: {
                    ...draft.investment_metrics,
                    [key]: { ...metric, ...patch },
                  },
                });
              return (
                <div key={key} className="rounded-xl border p-4">
                  <div className="mb-3 text-sm font-semibold">
                    {t(
                      `dashboard.listingForm.${key === "rental_yield" ? "rentalYield" : "pricePerM2"}`,
                    )}
                  </div>
                  <div className="grid gap-3 md:grid-cols-3">
                    <div>
                      <Label>{t("dashboard.listingForm.metricSource")}</Label>
                      <Input
                        value={metric.source}
                        onChange={(event) =>
                    ...
                    <div>
                      <Label>{t("dashboard.listingForm.metricAsOf")}</Label>
                      <Input
                        type="date"
                        value={metric.as_of}
                        onChange={(event) =>
                    ...
                    <div>
                      <Label>{t("dashboard.listingForm.metricStatus")}</Label>
                      <Select
                        value={metric.status}
                        onValueChange={(value) =>
                          update({ status: value as "actual" | "estimate" })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                        ...
                        <SelectContent>
                          <SelectItem value="actual">
                            {t("dashboard.listingForm.metricActual")}
                          </SelectItem>
                          <SelectItem value="estimate">
                            {t("dashboard.listingForm.metricEstimate")}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  ...
                  <label className="mt-3 flex items-center gap-2 text-sm">
                    ...
                    />
                    {t("dashboard.listingForm.metricHidden")}
                  </label>
                </div>
              );
            })}
            <div className="space-y-2">
              <Label>{t("dashboard.listingForm.roiNotes")}</Label>
              <Textarea
                value={draft.roi_notes || ""}
                onChange={(event) =>
                  onDraftChange({ roi_notes: event.target.value })
                }
              />
            </div>
          </section>
          <section className="space-y-3 rounded-2xl border bg-white p-4 md:col-span-2">
            <div className="font-semibold">
              {t("dashboard.seo.listingTitle")}
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    onDraftChange({
                      seo: {
                        ...draft.seo,
                        title: { ...draft.seo?.title, tr: event.target.value },
                      },
                    })
                  }
                />
                ...
                <Input
                  ...
                  onChange={(event) =>
                    onDraftChange({
                      seo: {
                        ...draft.seo,
                        title: { ...draft.seo?.title, en: event.target.value },
                      },
                    })
                  }
                />
            ...
            <div>
              ...
              <Input
                ...
                onChange={(event) =>
                  onDraftChange({
                    seo: { ...draft.seo, og_image: event.target.value },
                  })
                }
              />
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={draft.seo?.robots_index !== false}
                onChange={(event) =>
                  onDraftChange({
                    seo: { ...draft.seo, robots_index: event.target.checked },
                  })
                }
              />
        ...
        {generatedCopy ? (
          <div className="grid gap-4 md:grid-cols-2">
            <div className="flex flex-col rounded-2xl border border-[#173f32]/10 bg-white p-4">
              <div className="text-sm font-semibold">
                {t("dashboard.listingForm.platformStyle")}
              </div>
              <p className="mt-3 whitespace-pre-line text-sm leading-6 text-[#5f6c65]">
                {generatedCopy.platform_style}
              </p>
              <Button
                type="button"
                variant="outline"
                onClick={() =>
                  onDraftChange({ description: generatedCopy.platform_style })
                }
                className="mt-4 self-start"
              >
                {t("dashboard.listingForm.use")}
              </Button>
            </div>
            <div className="flex flex-col rounded-2xl border border-[#173f32]/10 bg-white p-4">
              <div className="text-sm font-semibold">
                {t("dashboard.listingForm.seoStyle")}
              </div>
              <p className="mt-3 whitespace-pre-line text-sm leading-6 text-[#5f6c65]">
                {generatedCopy.seo_style}
              </p>
              <Button
                type="button"
                variant="outline"
                onClick={() =>
                  onDraftChange({ description: generatedCopy.seo_style })
                }
                className="mt-4 self-start"
              >
                {t("dashboard.listingForm.use")}
        ...
        {socialKitOpen ? (
          <div className="rounded-2xl border border-[#173f32]/10 bg-white p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-semibold">
                  {t("dashboard.listingForm.socialTitle")}
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  {t("dashboard.listingForm.socialBody")}
                </p>
              </div>
              {socialKitLoading ? (
              ...
              <div className="mt-4 grid gap-5 md:grid-cols-2">
                <div>
                  <div className="text-sm font-medium">
                    {t("dashboard.listingForm.post")} · 1080×1080
                  </div>
                  <img
                    src={socialKitUrls.post}
                    alt={t("dashboard.listingForm.previewAlt", {
                      format: t("dashboard.listingForm.post"),
                    })}
                    className="mt-2 aspect-square w-full rounded-xl bg-slate-100 object-contain"
                  />
                  <Button
                    asChild
                    variant="outline"
                  ...
                  >
                    <a
                      href={socialKitUrls.post}
                      download={`${draft.id}-post.png`}
                    >
                      <Download className="h-4 w-4" />
                      {t("dashboard.listingForm.download")}
                    </a>
                  </Button>
                </div>
                <div>
                  <div className="text-sm font-medium">
                    {t("dashboard.listingForm.story")} · 1080×1920
                  </div>
                  <div className="mt-2 flex justify-center rounded-xl bg-slate-100 p-3">
                    <img
                      src={socialKitUrls.story}
                      alt={t("dashboard.listingForm.previewAlt", {
                        format: t("dashboard.listingForm.story"),
                      })}
                      className="aspect-[9/16] max-h-[430px] w-auto rounded-lg object-contain"
                    />
                  </div>
                  <Button
                    asChild
                  ...
                  >
                    <a
                      href={socialKitUrls.story}
                      download={`${draft.id}-story.png`}
                    >
                      <Download className="h-4 w-4" />
                      {t("dashboard.listingForm.download")}
                    </a>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {t(
              isSaving
                ? "dashboard.listingForm.saving"
                : "dashboard.listingForm.save",
            )}
          </Button>
          ...
          {draft.id ? (
            <Button
              type="button"
              variant="outline"
              onClick={() => void handleSocialKit()}
              disabled={socialKitLoading}
              className="gap-2"
            >
              <Sparkles className="h-4 w-4" />
              {t("dashboard.listingForm.socialKit")}
            </Button>
          ) : null}
          {draft.id ? (
            <Button
...
export function LandingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/platform-content?locale=${locale}`, {
      ...
      .then((body) => {
        ...
        if (typeof heroImageUrl === "string" && heroImageUrl)
          setPlatformHeroImage(heroImageUrl);
        setPlatformContentVersion((value) => value + 1);
      })
      .catch((error) => {
        if (error.name !== "AbortError")
          console.error("Platform content could not be loaded", error);
      });
  ...
  useEffect(() => {
    ...
    if (!prompt.trim() || translatedExamples.includes(prompt))
  ...
  const continueWithDesign = () => {
    ...
    void trackExperimentEvent(
      undefined,
      "prompt_quality_ready",
      `score_${promptQuality.score}`,
    ).catch(() => undefined);
    if (user) {
      navigate("/auth");
      return;
    ...
    <div className="min-h-screen overflow-hidden bg-[#f4f1ea] text-[#17231e]">
      <header className="relative z-30 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-10">
        ...
        <nav className="hidden items-center gap-8 text-sm font-medium text-[#48564f] md:flex">
          <a
            href="#nasil-calisir"
            className="transition-colors hover:text-[#173f32]"
          >
            {t("landing.nav.how")}
          </a>
          <a
          ...
          <a href="#sss" className="transition-colors hover:text-[#173f32]">
            {t("landing.nav.faq")}
          </a>
        </nav>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="hidden rounded-full text-[#25372f] sm:inline-flex"
          >
            {t("landing.nav.login")}
          </Button>
          <Button
            onClick={() =>
              document.getElementById("site-olustur")?.scrollIntoView()
            }
            className="rounded-full bg-[#173f32] px-5 text-white shadow-sm hover:bg-[#0f3025]"
          >
      ...
      <main>
        <section
          id="site-olustur"
          className="relative mx-auto max-w-7xl px-5 pb-20 pt-14 sm:px-8 sm:pt-20 lg:px-10 lg:pb-28"
        >
          ...
          <div className="relative grid items-center gap-14 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16">
            <div>
              ...
              <h1 className="max-w-3xl text-[3.55rem] font-semibold leading-[0.96] tracking-[-0.055em] text-[#13231c] sm:text-[4.8rem] lg:text-[5.4rem]">
                ...
                <span className="italic text-[#a35f3d]">
              ...
              <Card className="mt-9 max-w-xl rounded-[1.75rem] border-[#173f32]/10 bg-white/90 shadow-[0_24px_70px_rgba(47,58,51,0.10)] backdrop-blur">
                <CardContent className="p-3">
                  <Label htmlFor="business-prompt" className="sr-only">
                  ...
                  <div
                    className="px-3 pb-3"
                    aria-labelledby="prompt-guidance-title"
                  >
                    <div
                      id="prompt-guidance-title"
                      className="text-xs font-semibold text-[#354b41]"
                    >
                      {t("landing.hero.guidance.title")}
                    </div>
                    <div className="mt-2 flex gap-2 overflow-x-auto pb-1 sm:flex-wrap">
                      {(
                        ...
                        <button
                          key={dimension}
                          type="button"
                          onClick={() => applyPromptSuggestion(dimension)}
                          className={cn(
                            "shrink-0 rounded-full border px-3 py-1.5 text-xs transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#173f32]",
                            promptQuality.present[dimension]
                              ? "border-emerald-200 bg-emerald-50 text-emerald-800"
                              : "border-[#173f32]/15 bg-white text-[#42564d] hover:bg-[#edf1eb]",
                          )}
                          aria-pressed={promptQuality.present[dimension]}
                        >
                          {promptQuality.present[dimension] ? "✓ " : "+ "}
                          {t(`landing.hero.guidance.${dimension}`)}
                        </button>
                      ))}
                    </div>
                    <p role="status" className="mt-2 text-xs text-[#66756d]">
                      {promptQuality.score === 4
                        ? t("landing.hero.guidance.complete")
                        : t("landing.hero.guidance.missing", {
                            items: promptQuality.missing
                              .map((item) => t(`landing.hero.guidance.${item}`))
                              .join(", "),
                          })}
                    </p>
                  </div>
                  <div className="flex flex-col gap-3 border-t border-[#173f32]/10 px-1 pt-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="px-3 text-xs leading-5 text-[#758078]">
                      {t("landing.hero.promptHelp")}
                    </div>
                    <Button
                      onClick={continueWithDesign}
                      className="h-12 rounded-full bg-[#d86f45] px-5 text-white shadow-[0_10px_24px_rgba(216,111,69,0.24)] hover:bg-[#c55f38]"
                    >
                      {t("landing.hero.create")}
              ...
              <div
                className="mt-4 grid max-w-xl gap-2 rounded-2xl border border-[#173f32]/10 bg-white/60 p-4 text-xs leading-5 text-[#596860] sm:grid-cols-3"
                aria-label={t("landing.hero.expectations.title")}
              >
                <div>
                  <strong className="block text-[#173f32]">
                    1. {t("landing.hero.expectations.accountTitle")}
                  </strong>
                  {t("landing.hero.expectations.accountBody")}
                </div>
                <div>
                  <strong className="block text-[#173f32]">
                    2. {t("landing.hero.expectations.timeTitle")}
                  </strong>
                  {t("landing.hero.expectations.timeBody")}
                </div>
                <div>
                  <strong className="block text-[#173f32]">
                    3. {t("landing.hero.expectations.draftTitle")}
                  </strong>
                  {t("landing.hero.expectations.draftBody")}
                </div>
              </div>

              <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-[#647069]">
                {[
                  t("landing.hero.benefits.custom"),
                  t("landing.hero.benefits.ready"),
                  t("landing.hero.benefits.editable"),
                ].map((item) => (
                  <span key={item} className="inline-flex items-center gap-2">
                    <Check className="h-4 w-4 text-[#3b725d]" />
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div className="relative mx-auto w-full max-w-[680px]">
              ...
              <div className="relative overflow-hidden rounded-[2.1rem] border border-white/80 bg-white shadow-[0_35px_100px_rgba(40,55,47,0.20)]">
                <div className="flex items-center justify-between border-b border-[#19372d]/10 bg-[#fbfaf7] px-5 py-3">
                  <div className="flex gap-1.5">
                    <span className="h-2.5 w-2.5 rounded-full bg-[#e9a58b]" />
                    <span className="h-2.5 w-2.5 rounded-full bg-[#e6cf86]" />
                    <span className="h-2.5 w-2.5 rounded-full bg-[#9dc4a9]" />
                  </div>
                  <Badge className="rounded-full bg-[#e4f0e9] px-2.5 text-[10px] text-[#326049] hover:bg-[#e4f0e9]">
                    {t("landing.management.live")}
                  </Badge>
                </div>
                <div
                  className="bg-[#f6efe6] p-3 sm:p-4"
                  style={getThemeStyles(theme)}
                >
                  <div className="relative min-h-[285px] overflow-hidden rounded-[1.6rem] bg-[#243b32] text-white">
                    <img
                      data-platform-image="heroImageUrl"
                    ...
                    <div className="relative flex items-center justify-between px-5 py-4 text-[10px] sm:px-7">
                      <span className="font-semibold tracking-[0.18em]">
                        {previewBusinessName.toUpperCase()}
                      </span>
                      <div className="hidden items-center gap-4 text-white/80 sm:flex">
                        <span>{t("landing.preview.navListings")}</span>
                        <span>{t("landing.preview.navAbout")}</span>
                        <span>{t("landing.preview.navContact")}</span>
                      </div>
                    </div>
                    <div className="relative max-w-md px-5 pb-24 pt-7 sm:px-7 sm:pt-10">
                      ...
                      <div
                        className="mt-3 text-3xl font-semibold leading-[1.02] sm:text-4xl"
                        style={{ fontFamily: theme.fontPairing.heading }}
                      >
                        {t("landing.preview.headline")}
                      </div>
                      <p className="mt-3 max-w-sm text-xs leading-5 text-white/80">
                    ...
                    <div className="absolute inset-x-4 bottom-4 grid grid-cols-[1fr_1fr_auto] gap-2 rounded-2xl bg-[#fffaf3]/95 p-2.5 text-[#17231e] shadow-xl backdrop-blur sm:inset-x-7">
                      <div className="rounded-xl bg-white px-3 py-2">
                        <div className="text-[8px] uppercase tracking-[0.14em] text-[#8b776b]">
                          {t("landing.preview.neighborhood")}
                        </div>
                        <div className="mt-1 text-[10px] font-semibold">
                          {t("landing.preview.neighborhoodValue")}
                        </div>
                      </div>
                      <div className="rounded-xl bg-white px-3 py-2">
                        <div className="text-[8px] uppercase tracking-[0.14em] text-[#8b776b]">
                          {t("landing.preview.feeling")}
                        </div>
                        <div className="mt-1 text-[10px] font-semibold">
                          {t("landing.preview.feelingValue")}
                        </div>
                      </div>
                      <div className="grid place-items-center rounded-xl bg-[#d86f45] px-4 text-[10px] font-semibold text-white">
                  ...
                  <div className="px-1 pb-2 pt-5">
                    <div className="mb-3 flex items-end justify-between">
                      <div>
                        <div className="text-[9px] uppercase tracking-[0.18em] text-[#a35f3d]">
                          {t("landing.preview.matchEyebrow")}
                        </div>
                        <div className="mt-1 text-lg font-semibold text-[#17231e]">
                          {t("landing.preview.matchTitle")}
                        </div>
                      </div>
                      <span className="text-[10px] font-medium text-[#5d6962]">
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                      {previewListings.map((listing) => (
                        <div
                          key={listing.id}
                          className="overflow-hidden rounded-xl border border-[#17231e]/10 bg-white"
                        >
                          <img
                            src={listing.media[0]?.thumbUrl}
                            alt={listing.title}
                            className="aspect-[4/3] w-full object-cover"
                          />
                          <div className="p-2.5">
                            <div className="truncate text-[10px] font-semibold text-[#18251f]">
                              {listing.title}
                            </div>
                            <div className="mt-1 text-[9px] text-[#78837c]">
              ...
              <div className="absolute -bottom-7 -left-5 hidden rounded-2xl border border-white bg-white/95 p-4 shadow-[0_18px_45px_rgba(39,55,47,0.18)] backdrop-blur sm:block">
                <div className="flex items-center gap-3">
                  <div className="grid h-9 w-9 place-items-center rounded-full bg-[#e7efe8] text-[#315d4b]">
                    <Sparkles className="h-4 w-4" />
                  </div>
                  <div>
                    ...
                    <div className="mt-0.5 text-[10px] text-[#758078]">
                      {t("landing.preview.tailored")} · {previewRegion}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {[
              {
                ...
                text: t("landing.steps.oneBody"),
              },
              {
                value: "02",
                title: t("landing.steps.twoTitle"),
                text: t("landing.steps.twoBody"),
              },
              {
                value: "03",
                title: t("landing.steps.threeTitle"),
                text: t("landing.steps.threeBody"),
              },
            ].map((item) => (
              <div
                key={item.value}
                className="flex gap-5 py-8 sm:px-7 sm:first:pl-0 sm:last:pr-0"
              >
                <span className="font-serif text-2xl italic text-[#b46a48]">
                  {item.value}
                </span>
                <div>
                  <div className="font-semibold text-[#1b2a23]">
                    {item.title}
                  </div>
        ...
        >
          <div className="grid gap-14 lg:grid-cols-[0.72fr_1.28fr] lg:gap-24">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#a35f3d]">
                {t("landing.value.eyebrow")}
              </div>
              <h2 className="mt-5 text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl">
                {t("landing.value.title")}
              </h2>
              <p className="mt-6 text-base leading-7 text-[#66726b]">
                {t("landing.value.intro")}
              </p>
              <Button
                variant="outline"
                onClick={() => navigate(`/site/${demoSubdomain}`)}
                className="mt-8 rounded-full border-[#173f32]/20 bg-transparent px-5 text-[#173f32] hover:bg-white"
              >
                {t("landing.value.demo")}{" "}
                <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
            <div className="grid gap-5 sm:grid-cols-2">
              {[
                {
                  icon: Sparkles,
                  title: t("landing.value.siteTitle"),
                  text: t("landing.value.siteBody"),
                  tone: "bg-[#e5eee6] text-[#315d4b]",
                },
                {
                  icon: Home,
                  title: t("landing.value.listingTitle"),
                  text: t("landing.value.listingBody"),
                  tone: "bg-[#f4e4dc] text-[#a35f3d]",
                },
                {
                  icon: FileText,
                  title: t("landing.value.copyTitle"),
                  text: t("landing.value.copyBody"),
                  tone: "bg-[#ece7d8] text-[#766328]",
                },
                {
                  icon: Users,
                  title: t("landing.value.leadTitle"),
                  text: t("landing.value.leadBody"),
                  tone: "bg-[#e6e9ef] text-[#415477]",
                },
              ].map(({ icon: Icon, title, text, tone }) => (
                <Card
                  key={title}
                  className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7] shadow-none transition-transform duration-300 hover:-translate-y-1"
                >
        ...
        <section
          id="ozellikler"
          className="bg-[#173f32] py-24 text-white lg:py-32"
        >
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="mx-auto max-w-3xl text-center">
              <Badge className="rounded-full border-white/15 bg-white/10 px-4 py-2 text-white hover:bg-white/10">
                {t("landing.management.eyebrow")}
              </Badge>
              <h2 className="mt-6 text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl">
            ...
            <div className="mt-16 grid gap-6 lg:grid-cols-[1.18fr_0.82fr]">
                ...
                <div className="rounded-[1.65rem] border border-[#173f32]/10 bg-white p-4 sm:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs text-[#7a857e]">
                        {t("landing.management.listings")}
                      </div>
                      <div className="mt-1 text-xl font-semibold">
                        {t("landing.management.activeListings")}
                      </div>
                    </div>
                    <Button
                      size="sm"
                      className="rounded-full bg-[#173f32] text-white hover:bg-[#173f32]"
                    >
                      {" "}
                      <Plus className="mr-1 h-3.5 w-3.5" />{" "}
                      {t("landing.management.newListing")}
                    </Button>
                  </div>
                  <div className="mt-5 space-y-2.5">
                    {previewListings.map((listing) => (
                      <div
                        key={listing.id}
                        className="grid grid-cols-[52px_1fr_auto] items-center gap-3 rounded-2xl border border-[#173f32]/8 bg-[#fbfaf7] p-2.5"
                      >
                        <img
                          src={listing.media[0]?.thumbUrl}
                          alt=""
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-[11px] text-[#7a857e]">
                            {formatListingLocation(listing)} · {listing.m2} m² ·{" "}
                            {listing.room_count}
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <Badge className="mt-1 bg-[#e4f0e9] text-[9px] text-[#326049] hover:bg-[#e4f0e9]">
                            {t("landing.management.published")}
                          </Badge>
                        </div>
                      </div>
                    ))}
              ...
              <div className="grid gap-6">
                <Card className="rounded-[2.25rem] border-white/10 bg-white/10 text-white shadow-none">
                  <CardContent className="p-8">
                    <div className="flex items-start justify-between">
                      <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#d86f45]">
                        <FileText className="h-5 w-5" />
                      </div>
                      <Sparkles className="h-5 w-5 text-white/40" />
                    </div>
                    <h3 className="mt-10 text-3xl font-semibold">
                      {t("landing.management.copyReady")}
                    </h3>
                    <p className="mt-3 leading-7 text-white/65">
                      {t("landing.management.copyBody")}
                    </p>
                  </CardContent>
                </Card>
                <Card className="rounded-[2.25rem] border-white/10 bg-[#dbe5d2] text-[#173f32] shadow-none">
                  <CardContent className="p-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-xs uppercase tracking-[0.18em] text-[#66786e]">
                          {t("landing.management.newLead")}
                        </div>
                        <div className="mt-2 text-xl font-semibold">
                          {t("landing.management.sampleName")}
                        </div>
                      </div>
                    ...
                    <p className="mt-8 rounded-2xl bg-white/60 p-4 text-sm leading-6">
                      {t("landing.management.sampleMessage")}
                    </p>
                  </CardContent>
                </Card>
              </div>
        ...
        >
          <div className="overflow-hidden rounded-[2.75rem] border border-[#173f32]/10 bg-[#ebe7de] p-7 sm:p-10 lg:p-14">
            <div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
              <div>
                <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#a35f3d]">
                  {t("landing.personal.eyebrow")}
                </div>
                <h2 className="mt-5 text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl">
                  {t("landing.personal.title")}
                </h2>
                <p className="mt-6 max-w-xl text-base leading-7 text-[#66726b]">
                  {t("landing.personal.body")}
                </p>
                <Button
                  onClick={() =>
                    document
                      .getElementById("site-olustur")
                      ?.scrollIntoView({ behavior: "smooth" })
                  }
                  className="mt-8 h-12 rounded-full bg-[#173f32] px-6 text-white hover:bg-[#0f3025]"
                >
                  {t("landing.personal.start")}{" "}
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </div>
              <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
                {[
                  {
                    icon: MapPin,
                    title: t("landing.personal.regionTitle"),
                    text: t("landing.personal.regionBody"),
                  },
                  {
                    icon: Sparkles,
                    title: t("landing.personal.toneTitle"),
                    text: t("landing.personal.toneBody"),
                  },
                  {
                    icon: FileText,
                    title: t("landing.personal.contentTitle"),
                    text: t("landing.personal.contentBody"),
                  },
                ].map(({ icon: Icon, title, text }) => (
                  <Card
                    key={title}
                    className="rounded-[1.5rem] border-[#173f32]/10 bg-white/70 shadow-none"
                  >
                    <CardContent className="flex gap-4 p-5">
                      <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#dbe5d2] text-[#315d4b]">
                        <Icon className="h-5 w-5" />
                      </div>
                      <div>
        ...
        <section className="px-5 pb-24 sm:px-8 lg:px-10 lg:pb-32">
          <div className="mx-auto max-w-7xl overflow-hidden rounded-[2.75rem] bg-[#d86f45] px-6 py-16 text-center text-white shadow-[0_28px_80px_rgba(216,111,69,0.22)] sm:px-12 lg:py-20">
            ...
            <h2 className="mx-auto mt-7 max-w-3xl text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl">
              {t("landing.cta.title")}
            </h2>
            ...
            <Button
              ...
              className="mt-8 h-12 rounded-full bg-white px-6 text-[#9e4e30] hover:bg-[#fff8f4]"
            >
        ...
        <section id="sss" className="border-t border-[#173f32]/10 bg-[#ebe7de]">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.7fr_1.3fr] lg:px-10 lg:py-24">
            ...
            <div className="divide-y divide-[#173f32]/12 border-y border-[#173f32]/12">
              {[1, 2, 3, 4]
                .map((number) => ({
                  q: t(`landing.faq.q${number}`),
                  a: t(`landing.faq.a${number}`),
                }))
                .map((item) => (
                  <details key={item.q} className="group py-6">
                    ...
                    <p className="max-w-2xl pt-4 text-sm leading-7 text-[#66726b]">
                      {item.a}
                    </p>
                  </details>
                ))}
      ...
      <footer className="bg-[#13271f] text-white">
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-10">
          <div className="flex flex-col gap-10 border-b border-white/10 pb-10 md:flex-row md:items-start md:justify-between">
            ...
            <div className="grid grid-cols-2 gap-x-16 gap-y-3 text-sm text-white/65">
              ...
              <a href="#size-ozel" className="hover:text-white">
                {t("landing.nav.personal")}
              </a>
          ...
          <div className="flex flex-col gap-3 pt-8 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span>{t("landing.footer.market")}</span>
          </div>
        </div>
      </footer>
    </div>
...
export function AuthPage() {
  ...
  const previewListings = state.listings
    .filter((listing) => listing.site_id === "site_demo")
    .slice(0, 3);
  const displayConfig = {
  ...
  const handleGenerateSite = useCallback(async () => {
    if (!session || generationStarted.current || prompt.trim().length < 10)
      return;
    ...
    void trackExperimentEvent(
      ...
      "onboarding_generation_start",
      `quality_${evaluatePrompt(prompt).score}`,
    ).catch(() => undefined);
    try {
      const response = await fetch("/api/generate-theme", {
        ...
        body: JSON.stringify({
          prompt,
          preferences: {
            templateId: templatePreference || understood.templateId,
            audience: audiencePreference || understood.audience,
            region: regionPreference || understood.region,
          },
        }),
      });
      ...
      if (!response.ok)
        throw new Error(payload.error || t("auth.resume.generationError"));
      if (!payload.site_id) throw new Error(t("auth.resume.missingId"));
      if (!payload.slug || !payload.public_path)
        throw new Error(t("auth.resume.missingUrl"));
  ...
  return (
    <div className="min-h-screen bg-[#f4f1ea] text-[#17231e]">
      <header className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 sm:px-8 lg:px-10">
        <Link to="/" className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-full bg-[#173f32] text-white">
            <Home className="h-[18px] w-[18px]" />
          </div>
          <span className="text-lg font-bold tracking-[-0.03em]">
            {t("common.brand")}
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            to="/"
            className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm text-[#66726b] transition hover:bg-white"
          >
            <ArrowLeft className="h-4 w-4" />
            {t("auth.resume.backHome")}
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-[1440px] gap-10 px-5 pb-12 pt-6 sm:px-8 lg:min-h-[calc(100vh-90px)] lg:grid-cols-[0.78fr_1.22fr] lg:items-center lg:px-10 lg:pb-16">
        <div className="mx-auto w-full max-w-xl">
          <div className="mb-8 flex items-center gap-2">
            {[
              { no: "01", label: t("auth.resume.design"), done: true },
              { no: "02", label: t("auth.resume.details"), done: false },
              { no: "03", label: t("auth.resume.publish"), done: false },
            ].map((step, index) => (
              <div key={step.no} className="flex items-center gap-2">
                <div
                  className={cn(
                    "grid h-8 w-8 place-items-center rounded-full text-[10px] font-semibold",
                    step.done || index === 1
                      ? "bg-[#173f32] text-white"
                      : "border border-[#173f32]/15 bg-white/50 text-[#7b857f]",
                  )}
                >
                  {step.done ? <Check className="h-3.5 w-3.5" /> : step.no}
                </div>
                <span
                  className={cn(
                    "hidden text-xs sm:block",
                    index === 1
                      ? "font-semibold text-[#173f32]"
                      : "text-[#8a948e]",
                  )}
                >
                  {step.label}
                ...
                {index < 2 ? (
                  <div className="mx-1 h-px w-5 bg-[#173f32]/15 sm:w-8" />
                ) : null}
              </div>
            ))}
          </div>

          <Badge className="rounded-full border border-[#173f32]/10 bg-white/65 px-4 py-2 text-[#173f32] shadow-sm hover:bg-white/65">
            <Sparkles className="mr-2 h-3.5 w-3.5" />
            {t("auth.resume.title")}
          </Badge>
          <h1 className="mt-6 text-5xl font-semibold leading-[1.02] tracking-[-0.05em] sm:text-6xl">
            {t("auth.resume.subtitle")}
          </h1>
          <p className="mt-5 max-w-lg text-base leading-7 text-[#66726b]">
            {t("auth.resume.body")}
          </p>
          ...
          <Card className="mt-8 rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7] shadow-[0_22px_65px_rgba(39,52,45,0.10)]">
            <CardContent className="space-y-5 p-6 sm:p-7">
              <div className="space-y-2">
                <Label htmlFor="auth-prompt">
                  {t("auth.resume.promptLabel")}
                </Label>
                <Textarea
                  id="auth-prompt"
                  value={prompt}
                  onChange={(event) => setPrompt(event.target.value)}
                  className="min-h-28 resize-none rounded-xl border-[#173f32]/10 bg-white leading-6"
                />
              </div>
              <div className="rounded-2xl border border-[#173f32]/10 bg-[#edf1eb] p-4">
                <div className="text-xs font-semibold uppercase tracking-wider text-[#527064]">
                  {t("auth.resume.understoodTitle")}
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div>
                    <Label>{t("auth.resume.audience")}</Label>
                    <Input
                      value={audiencePreference || understood.audience}
                      onChange={(event) =>
                  ...
                  <div>
                    <Label>{t("auth.resume.region")}</Label>
                    <Input
                      value={regionPreference || understood.region}
                      onChange={(event) =>
                        setRegionPreference(event.target.value)
                      }
                    />
                  </div>
                </div>
                <div className="mt-3">
                  <Label>{t("auth.resume.theme")}</Label>
                  <Select
                    value={templatePreference || understood.templateId}
                    onValueChange={setTemplatePreference}
                  >
                    <SelectTrigger className="mt-2 bg-white">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {selectableTemplateIds.map((id) => (
                        <SelectItem key={id} value={id}>
                          {t(`dashboard.templateSwitch.templates.${id}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <p className="mt-3 text-xs leading-5 text-[#607068]">
                  {t("auth.resume.understoodHelp")}
                </p>
              </div>
              {generationError ? (
                <div
              ...
              <Button
                className="h-12 w-full rounded-full bg-[#d86f45] text-white shadow-[0_10px_24px_rgba(216,111,69,0.22)] hover:bg-[#c76039]"
                onClick={() => {
                  generationStarted.current = false;
              ...
              >
                ...
                />
                {isGenerating
                  ? t("auth.resume.generating")
                  : generationError
                    ? t("auth.resume.retry")
                    : t("auth.resume.generate")}
              </Button>
              <div className="flex items-center justify-center gap-2 text-[11px] text-[#7d8781]">
                <Check className="h-3.5 w-3.5 text-[#3b725d]" />
                {t("auth.resume.cacheNote")}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="relative mx-auto w-full max-w-[760px] lg:pl-6">
          ...
          <div className="relative overflow-hidden rounded-[2.35rem] border-[9px] border-[#222a26] bg-white shadow-[0_35px_100px_rgba(38,53,45,0.20)]">
            <div className="flex items-center justify-between border-b border-[#173f32]/10 bg-[#f7f5f0] px-5 py-3">
              <div className="flex gap-1.5">
                <span className="h-2.5 w-2.5 rounded-full bg-[#e9a58b]" />
                <span className="h-2.5 w-2.5 rounded-full bg-[#e6cf86]" />
                <span className="h-2.5 w-2.5 rounded-full bg-[#9dc4a9]" />
              </div>
              <div className="rounded-full bg-white px-4 py-1.5 text-[10px] text-[#748079]">
                {t("auth.resume.preview")}
              </div>
              <Badge className="rounded-full bg-[#e0eee5] px-2.5 text-[9px] text-[#326049] hover:bg-[#e0eee5]">
                {t("auth.resume.ready")}
              </Badge>
            </div>
            <div className="p-4 sm:p-5" style={getThemeStyles(preview.theme)}>
              <div
                className="overflow-hidden rounded-[1.8rem] text-white"
                style={{
                  background: `linear-gradient(145deg, ${displayConfig.primary_color}, ${displayConfig.accent_color})`,
                }}
              >
                <div className="flex items-center justify-between px-6 py-4 text-[9px] font-semibold uppercase tracking-[0.18em]">
                  <span>{displayConfig.business_name}</span>
                  <div className="hidden gap-4 font-normal tracking-normal text-white/70 sm:flex">
                    <span>{t("landing.preview.navListings")}</span>
                    <span>{t("landing.preview.navAbout")}</span>
                    <span>{t("landing.preview.navContact")}</span>
                  </div>
                </div>
                <div className="grid items-end gap-6 px-6 pb-8 pt-12 sm:grid-cols-[1fr_0.7fr] sm:px-8 sm:pb-10 sm:pt-16">
                  <div>
                    <div className="text-[9px] uppercase tracking-[0.2em] text-white/55">
                      {preview.profile.region_focus}
                    </div>
                    <h2
                      className="mt-3 text-4xl font-semibold leading-none"
                      style={{ fontFamily: preview.theme.fontPairing.heading }}
                    >
                      {displayConfig.headline}
                    </h2>
                  ...
                  <div className="hidden rounded-2xl border border-white/20 bg-white/10 p-4 backdrop-blur sm:block">
                    <div className="text-[8px] uppercase tracking-[0.16em] text-white/50">
                      {t("auth.resume.brandTone")}
                    </div>
                    <div className="mt-2 text-sm font-semibold">
                      {displayConfig.tone}
                    </div>
                    <div className="mt-1 text-[10px] text-white/60">
                      {t("auth.resume.generatedBy")}
                    </div>
                  </div>
                </div>
              </div>
              <div className="px-1 pb-2 pt-6">
                <div className="mb-4 flex items-end justify-between">
                  <div>
                    <div className="text-[9px] uppercase tracking-[0.18em] text-[#839087]">
                      {t("landing.preview.currentListings")}
                    </div>
                    <div className="mt-1 text-xl font-semibold">
                      {t("landing.preview.discover")}
                    </div>
                  </div>
                  <span className="text-[10px] text-[#748079]">
                    {t("landing.preview.viewAll")} →
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {previewListings.map((listing) => (
                    <div
                      key={listing.id}
                      className="overflow-hidden rounded-xl border border-[#173f32]/10 bg-white"
                    >
                      <img
                        src={listing.media[0]?.thumbUrl}
                        alt={listing.title}
                      ...
                      <div className="p-2.5">
                        ...
                        <div className="mt-1 text-[9px] text-[#7a857e]">
                          {formatListingLocation(listing)} ·{" "}
                          {listing.room_count}
                        </div>
                      </div>
          ...
          <div className="relative mx-auto mt-5 flex max-w-lg items-center justify-center gap-5 text-xs text-[#6f7a73]">
            <span className="inline-flex items-center gap-2">
              <Sparkles className="h-3.5 w-3.5" />
              {t("auth.resume.tailored")}
            </span>
            <span className="h-3 w-px bg-[#173f32]/15" />
            <span className="inline-flex items-center gap-2">
...
export function GeneratedSitePreviewPage() {
  ...
  usePageMeta(
    config
      ? `${config.business_name} - Site Önizlemesi`
      : "Site önizlemesi yükleniyor",
    config?.headline
      ? generatedText(config.headline)
      : "Kaydedilmiş Fastate AI site önizlemesi.",
  );
  ...
  useEffect(() => {
    ...
    const loadSite = async () => {
      try {
        ...
        if (!response.ok)
          throw new Error(
            payload.error || "Site configuration could not be loaded.",
          );
      ...
      <div className="grid min-h-screen place-items-center bg-[#f4f1ea] px-5 text-[#17231e]">
        <Card className="w-full max-w-lg rounded-[2rem] border-[#173f32]/10 bg-white text-center shadow-sm">
          <CardContent className="p-8">
            ...
            <Button
              asChild
              className="mt-6 rounded-full bg-[#173f32] text-white"
            >
  ...
  if (!config) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#f4f1ea] text-sm text-[#66726b]">
        Kaydedilmiş site yükleniyor...
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f5f2eb] text-[#17231e]">
      <header className="border-b border-[#173f32]/10 bg-white/70 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-2">
            <Badge className="rounded-full bg-[#e0eee5] px-3 text-[#326049] hover:bg-[#e0eee5]">
              Kaydedilmiş taslak
            </Badge>
            {user ? (
              <Button asChild className="rounded-full bg-[#173f32]">
              ...
              <>
                ...
                <Button asChild className="rounded-full bg-[#d86f45]">
                  <Link to="/signup">Kaydol ve sahiplen</Link>
                </Button>
              </>
            )}
          </div>
      ...
      <main>
        <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10 lg:py-16">
          <div
            className="overflow-hidden rounded-[2.5rem] px-6 py-16 text-white shadow-[0_30px_90px_rgba(30,45,38,0.18)] sm:px-12 lg:px-16 lg:py-24"
            style={{
          ...
          >
            ...
            <p className="mt-6 max-w-xl text-base leading-7 text-white/75">
              Seçkin portföyler ve güvene dayalı kişisel gayrimenkul
              danışmanlığı.
            </p>
            <Button className="mt-8 rounded-full bg-white px-6 text-[#173f32] hover:bg-white/90">
              Portföyleri keşfedin <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-20 sm:px-8 lg:px-10">
          <div className="mb-7 flex items-end justify-between">
            <div>
              <div className="text-xs uppercase tracking-[0.2em] text-[#78847d]">
                Güncel portföyler
              </div>
              <h2 className="mt-2 text-3xl font-semibold">
                Öne çıkan yaşam alanları
              </h2>
            </div>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            {previewListings.map((listing) => (
              <Card
                key={listing.id}
                className="overflow-hidden rounded-[1.5rem] border-[#173f32]/10 bg-white shadow-none"
              >
                <img
                ...
                <CardContent className="p-5">
                  ...
                  <p className="mt-2 text-sm text-[#78837c]">
                    {formatListingLocation(listing)} · {listing.room_count}
                  </p>
                  <div
...
export function PublicSitePage() {
  ...
  useEffect(() => {
    ...
    const loadSite = async () => {
      try {
        ...
        const payload = await response.json();
        if (!response.ok)
          throw new Error(payload.error || "Site could not be loaded.");
  ...
  const site = publicSite
    ? {
        ...
        heroSubtitle:
          "Seçkin portföyler ve güvene dayalı kişisel gayrimenkul danışmanlığı.",
        theme_config: {
  ...
  const listings = state.listings.filter(
    (listing) =>
      listing.site_id === "site_demo" && listing.status !== "passive",
  );

  usePageMeta(
    site
      ? `${agent?.businessName || "Fastate AI"} - ${site.heroTitle}`
      : "Fastate AI - Site bulunamadı",
      ...
      listings.filter((listing) => {
        if (
          ...
          !formatListingLocation(listing)
            ...
            .includes(filters.district.toLocaleLowerCase("tr-TR"))
        )
          return false;
        if (filters.type !== "all" && listing.listing_type !== filters.type)
          return false;
        if (
          filters.query &&
  ...
  if (isLoadingSite) {
    return (
      <div className="grid min-h-screen place-items-center bg-[#f5f2ec] text-sm text-[#66726b]">
        Site yükleniyor...
      </div>
    );
  }
        ...
        <Card className="max-w-lg border-white/10 bg-white/5 text-white">
          <CardContent className="space-y-4 p-6 text-center">
            ...
            <p className="text-white/70">
              {siteLoadError || "Bu adres için tanımlı bir site yok."}
            </p>
  ...
  return (
    <div
      className="min-h-screen bg-[#f5f2ec] text-[#18251f]"
      style={getThemeStyles(site.theme_config)}
    >
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-5 py-6 sm:px-8 lg:px-10">
        <Link
          ...
          style={{ color: site.theme_config.primary }}
        >
          {agent.businessName}
        </Link>
        <nav className="hidden items-center gap-8 text-sm text-[#66726b] md:flex">
        ...
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className="hidden rounded-full border-[#173f32]/10 bg-white/70 text-[#66726b] sm:inline-flex"
          >
            /site/{site.subdomain}
          </Badge>
          <Button
            ...
            style={{ backgroundColor: site.theme_config.primary }}
          >
            <a href="#iletisim">
      ...
      <main>
        <section className="mx-auto max-w-7xl px-5 pb-20 pt-8 sm:px-8 lg:px-10 lg:pb-28 lg:pt-14">
          <div className="grid items-stretch gap-6 lg:grid-cols-[0.88fr_1.12fr]">
            <div
              className="flex flex-col justify-between rounded-[2.5rem] p-8 text-white sm:p-12"
              style={{
                background: `linear-gradient(145deg, ${site.theme_config.primary}, ${site.theme_config.accent})`,
              }}
            >
              <div>
                <div className="text-xs uppercase tracking-[0.24em] text-white/60">
                  {agent.region} · Gayrimenkul danışmanlığı
                </div>
                <h1
                  className="mt-8 text-5xl font-semibold leading-[0.98] tracking-[-0.045em] sm:text-6xl"
                  style={{ fontFamily: site.theme_config.fontPairing.heading }}
                >
                  {site.heroTitle}
                </h1>
              ...
              <div className="mt-14 flex flex-wrap items-center gap-4">
                <Button
                  asChild
                  className="rounded-full bg-white px-6 text-[#173f32] hover:bg-white/90"
                >
                  <a href="#portfoyler">
                    Portföyleri keşfedin <ArrowRight className="ml-2 h-4 w-4" />
                  </a>
                ...
                <div className="text-xs text-white/55">
                  Yerel uzmanlık · Kişisel danışmanlık
                </div>
              </div>
            </div>
            {featured ? (
              <Link
                to={`/site/${site.subdomain}/listings/${featured.id}`}
                className="group relative min-h-[480px] overflow-hidden rounded-[2.5rem] lg:min-h-[620px]"
              >
                <img
                ...
                <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                  <Badge className="rounded-full border-white/20 bg-white/15 text-white backdrop-blur hover:bg-white/15">
                    Öne çıkan portföy
                  </Badge>
                  <div className="mt-4 flex items-end justify-between gap-4">
                    <div>
                      <h2
                        ...
                        style={{
                          fontFamily: site.theme_config.fontPairing.heading,
                        }}
                      >
                        {featured.title}
                      </h2>
                      <div className="mt-2 text-sm text-white/70">
                        {featured.district} · {featured.room_count} ·{" "}
                        {featured.m2} m²
                      </div>
                    </div>
                    <div className="shrink-0 text-xl font-semibold sm:text-2xl">
                      {formatListingPrice(featured)}
                    </div>
                  </div>
                </div>
              </Link>
        ...
        <section
          id="portfoyler"
          className="border-y border-[#173f32]/10 bg-[#eeebe4] py-20 lg:py-28"
        >
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              <div>
                <div
                  ...
                  style={{ color: site.theme_config.accent }}
                >
                  Seçkin portföyler
                </div>
                <h2
                  className="mt-4 text-5xl font-semibold tracking-[-0.045em] sm:text-6xl"
                  style={{ fontFamily: site.theme_config.fontPairing.heading }}
                >
                  Size uygun yaşam alanını bulun.
                </h2>
              </div>
              <p className="max-w-md text-sm leading-6 text-[#69756e]">
                Her portföyü yerinde inceliyor, doğru bilgi ve şeffaf iletişimle
                sunuyoruz.
              </p>
            </div>
            ...
            <div className="mt-10 grid gap-3 rounded-[1.6rem] border border-[#173f32]/10 bg-white p-3 shadow-sm md:grid-cols-[1.25fr_0.8fr_0.8fr_0.8fr_auto]">
              <div className="relative">
                <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8a948e]" />
                <Input
                  ...
                  className="h-12 rounded-xl border-0 bg-[#f5f3ee] pl-11 shadow-none"
                />
              </div>
              <Input
                placeholder="Bölge"
                ...
                className="h-12 rounded-xl border-0 bg-[#f5f3ee] shadow-none"
              />
              <Select
                value={filters.type}
                onValueChange={(value) =>
              ...
              >
                <SelectTrigger className="h-12 rounded-xl border-0 bg-[#f5f3ee] shadow-none">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              />
              <div
                className="grid h-12 place-items-center rounded-xl px-5 text-sm text-white"
                style={{ backgroundColor: site.theme_config.primary }}
              >
                {filteredListings.length} portföy
              </div>
            </div>

            <div className="mt-10 grid gap-x-5 gap-y-10 md:grid-cols-2 lg:grid-cols-3">
              {filteredListings.map((listing) => (
                <Link
                  to={`/site/${site.subdomain}/listings/${listing.id}`}
                  key={listing.id}
                  className="group"
                >
                  <div className="relative overflow-hidden rounded-[2rem]">
                    ...
                    <div className="absolute left-4 top-4 flex gap-2">
                      <Badge className="rounded-full bg-white/90 text-[#173f32] backdrop-blur hover:bg-white">
                        {listing.listing_type === "sale"
                          ? "Satılık"
                          : "Kiralık"}
                      </Badge>
                      <Badge className="rounded-full bg-black/35 text-white backdrop-blur hover:bg-black/35">
                        {formatListingLocation(listing)}
                      </Badge>
                    </div>
                    <div className="absolute bottom-4 right-4 grid h-11 w-11 place-items-center rounded-full bg-white text-[#173f32] transition-transform group-hover:translate-x-1">
                  ...
                  <div className="px-1 pt-5">
                    ...
                    <div className="mt-3 flex items-center gap-4 text-xs text-[#738078]">
        ...
        <section
          id="hakkimda"
          className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-24 sm:px-8 lg:grid-cols-[0.85fr_1.15fr] lg:px-10 lg:py-32"
        >
          <div
            className="relative mx-auto aspect-[4/5] w-full max-w-md overflow-hidden rounded-[2.5rem]"
            style={{
              background: `linear-gradient(145deg, ${site.theme_config.primary}22, ${site.theme_config.accent}88)`,
            }}
          >
            <div className="absolute inset-8 flex flex-col justify-end rounded-[1.8rem] border border-white/40 bg-white/30 p-7 backdrop-blur-sm">
              <div
                className="grid h-20 w-20 place-items-center rounded-full bg-white text-2xl font-semibold"
                style={{ color: site.theme_config.primary }}
              >
                {agent.name
                  .split(" ")
              ...
              <div className="mt-2 text-sm text-[#5f7067]">
                Gayrimenkul danışmanı · {agent.region}
              </div>
            </div>
          </div>
          <div>
            ...
            <h2
              className="mt-5 max-w-2xl text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl"
              style={{ fontFamily: site.theme_config.fontPairing.heading }}
            >
              Doğru karar, güvenilir bir danışmanlıkla başlar.
            </h2>
            <p className="mt-7 max-w-xl text-base leading-8 text-[#657169]">
              {agent.bio} Her portföyde ihtiyaçlarınızı dinleyen, bölge
              bilgisini veriye dayalı değerlendirmelerle birleştiren kişisel bir
              yaklaşım sunuyorum.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Button
                variant="outline"
                className="rounded-full border-[#173f32]/15 bg-transparent px-6"
              >
        ...
        <section id="iletisim" className="px-5 pb-20 sm:px-8 lg:px-10">
          <div
            className="mx-auto grid max-w-7xl gap-8 overflow-hidden rounded-[2.75rem] p-8 text-white sm:p-12 lg:grid-cols-[0.9fr_1.1fr] lg:p-16"
            style={{ backgroundColor: site.theme_config.primary }}
          >
            <div>
              ...
              <h2
                ...
                style={{ fontFamily: site.theme_config.fontPairing.heading }}
              >
                Yeni evinizi birlikte bulalım.
              </h2>
              <p className="mt-4 max-w-md text-sm leading-7 text-white/65">
                Aradığınız portföyü tarif edin; size en kısa sürede kişisel
                olarak dönüş yapayım.
              </p>
            </div>
            <Card className="rounded-[2rem] border-0 bg-white text-[#17231e] shadow-none">
              <CardContent className="p-6 sm:p-7">
                <PublicContactForm siteId={site.id} />
              </CardContent>
            </Card>
          </div>
        </section>
      ...
      <footer className="border-t border-[#173f32]/10 bg-[#ece8df]">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-10 text-sm text-[#69756e] sm:px-8 md:flex-row md:items-center md:justify-between lg:px-10">
          <div>
            <div
          ...
          <div className="text-xs">Fastate AI ile hazırlandı</div>
        </div>
      </footer>
    </div>
  );
  ...
  return (
    ...
    >
      <header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-6 sm:px-8 lg:px-10">
        ...
        <Button
          variant="ghost"
          onClick={() => navigate(`/site/${site.subdomain}`)}
          className="rounded-full text-[#536159]"
        >
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10">
        <div className="mb-10 mt-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex flex-wrap gap-2">
              <Badge
              ...
              <Badge
                variant="outline"
                className="rounded-full border-[#173f32]/15 bg-white/50"
              >
                {formatListingLocation(listing)}
              </Badge>
            </div>
            <h1
              className="mt-5 max-w-4xl text-5xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-6xl"
              style={{ fontFamily: site.theme_config.fontPairing.heading }}
            >
              {listing.title}
            </h1>
          ...
          <div className="text-left lg:text-right">
            <div className="text-sm text-[#77827b]">Satış fiyatı</div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
          <img
            src={listing.media[0]?.url}
            alt={listing.title}
            className="aspect-[16/10] h-full w-full rounded-[2.5rem] object-cover"
          />
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-1">
            {listing.media.slice(1, 3).map((item) => (
              <img
                key={item.id}
                src={item.url}
                alt={item.alt}
                className="h-full min-h-0 w-full rounded-[2rem] object-cover"
              />
            ))}
            <div
              className="flex min-h-[150px] flex-col justify-end rounded-[2rem] p-6 text-white"
              style={{
                background: `linear-gradient(145deg, ${site.theme_config.primary}, ${site.theme_config.accent})`,
              }}
            >
              ...
              <div className="mt-2 flex items-center gap-2 text-xl font-semibold">
                ...
                {formatListingLocation(listing)}, İstanbul
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_380px] lg:items-start">
          <div>
            <div className="grid grid-cols-3 divide-x divide-[#173f32]/12 rounded-[1.8rem] border border-[#173f32]/10 bg-white/60 py-6 text-center">
              <div>
                <div className="text-xs text-[#7b857f]">Oda sayısı</div>
                <div className="mt-2 text-2xl font-semibold">
                  {listing.room_count}
                </div>
              </div>
              <div>
                <div className="text-xs text-[#7b857f]">Brüt alan</div>
                <div className="mt-2 text-2xl font-semibold">
                  {listing.m2} m²
                </div>
              </div>
              <div>
                <div className="text-xs text-[#7b857f]">İlan tipi</div>
                <div className="mt-2 text-2xl font-semibold">
                  {listing.listing_type === "sale" ? "Satılık" : "Kiralık"}
                </div>
              </div>
            </div>
            <section className="border-b border-[#173f32]/10 py-10">
              <div
              ...
              <h2
                ...
                style={{ fontFamily: site.theme_config.fontPairing.heading }}
              >
                Yaşam alanının detayları
              </h2>
              <p className="mt-5 max-w-3xl text-base leading-8 text-[#606d65]">
                {listing.description}
              </p>
            </section>
            <section className="border-b border-[#173f32]/10 py-10">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {listing.features.map((feature) => (
                  <div
                    key={feature}
                    className="flex items-center gap-3 rounded-2xl bg-[#ebece5] px-4 py-4 text-sm"
                  >
            ...
            <section className="py-10">
              ...
              <div className="mt-6 grid aspect-[16/7] place-items-center overflow-hidden rounded-[2rem] bg-[linear-gradient(135deg,#dde6dc,#e8dfd4)]">
                <div className="text-center">
                  ...
                  <div className="mt-1 text-xs text-[#718077]">
          ...
          <Card className="sticky top-6 rounded-[2rem] border-[#173f32]/10 bg-white shadow-[0_24px_70px_rgba(36,50,43,0.10)]">
            <CardHeader>
              <div className="flex items-center gap-3">
                <div
                  className="grid h-12 w-12 place-items-center rounded-full bg-[#dbe5d2] font-semibold"
                  style={{ color: site.theme_config.primary }}
                >
                  {agent.name
                    .split(" ")
                ...
                <div>
                  ...
                  <CardDescription>{agent.region} bölge uzmanı</CardDescription>
                </div>
              </div>
              <div className="pt-3">
                <CardTitle
                  ...
                  style={{ fontFamily: site.theme_config.fontPairing.heading }}
                >
                  Bu portföyle ilgileniyorum
                </CardTitle>
                <CardDescription className="mt-2 leading-6">
                  Bilgilerinizi bırakın, size kısa süre içinde dönüş yapalım.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              ...
              <Button
                variant="outline"
                asChild
                className="mt-3 w-full rounded-xl border-[#173f32]/12"
              >
                <a href={`tel:${agent.phone}`}>
                  <Phone className="mr-2 h-4 w-4" />
                  {agent.phone}
      ...
      <footer className="border-t border-[#173f32]/10 bg-[#ece8df]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-8 text-xs text-[#6f7b73] sm:px-8 lg:px-10">
          <span>{agent.businessName}</span>
          <span>Fastate AI ile hazırlandı</span>
        </div>
      </footer>
    </div>
  );
