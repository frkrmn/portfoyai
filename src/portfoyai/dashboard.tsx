...
import { ArrowRight, Globe, Home, Plus, RefreshCw, Users } from "lucide-react";
...
import { formatDateTR, formatTRY, generateDescriptionFromFacts } from "./mock";
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const payload = await readApiJson<{ error?: string; sites?: DashboardSite[] }>(response);
  ...
  return <Shell businessName={activeSite?.business_name || ""} activeSection={activeTab} onSectionChange={setActiveTab} leadCount={siteLeads.length} actions={<div className="flex items-center gap-2">{activeSite ? <Button variant="outline" asChild className="rounded-full border-[#173f32]/10 bg-white"><a href={`/site/${activeSite.slug}`} target="_blank" rel="noreferrer"><Globe className="mr-2 h-4 w-4" />Siteyi aç</a></Button> : null}<Button variant="outline" size="icon" title="Verileri yenile" onClick={() => void loadLeads()} className="rounded-full border-[#173f32]/10 bg-white"><RefreshCw className="h-4 w-4" /></Button></div>}>
    <div className="space-y-7">
      ...
      {activeSite && activeTab === "listings" ? <div className="grid gap-6 xl:grid-cols-[1fr_.9fr]"><Card className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7] shadow-none"><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Portföyler</CardTitle><CardDescription>{listings.length} gerçek DB kaydı</CardDescription></div><Button onClick={() => setDraft(blankListing(activeSite.id, siteDraft?.address || "İstanbul"))} className="rounded-full"><Plus className="mr-2 h-4 w-4" />Yeni</Button></CardHeader><CardContent className="space-y-3">{listings.map((listing) => <button key={listing.id} onClick={() => setDraft({ ...listing })} className={cn("grid w-full grid-cols-[72px_1fr_auto] items-center gap-4 rounded-2xl border p-3 text-left", draft.id === listing.id ? "border-[#173f32] bg-[#edf1eb]" : "bg-white")}><img src={getListingImage(listing)} alt="" className="h-16 w-[72px] rounded-xl object-cover" /><div className="min-w-0"><div className="truncate font-semibold">{listing.title}</div><div className="mt-1 text-xs text-[#7a857e]">{listing.district} · {listing.room_count} · {listing.m2} m²</div><div className="mt-2 text-sm font-semibold">{formatTRY(listing.price)}</div></div><Badge>{listing.listing_type === "sale" ? "Satılık" : "Kiralık"}</Badge></button>)}</CardContent></Card><ListingForm siteId={activeSite.id} draft={draft} onDraftChange={(patch) => setDraft((current) => ({ ...current, ...patch }))} onSave={() => void saveListing()} isSaving={savingListing} onGenerate={() => setDraft((current) => ({ ...current, description: generateDescriptionFromFacts(current) }))} onReset={() => setDraft(blankListing(activeSite.id, siteDraft?.address || "İstanbul"))} onDelete={() => void removeListing()} /></div> : null}

      {activeSite && activeTab === "leads" ? <Card className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7] shadow-none"><CardHeader className="flex-row items-center justify-between"><div><CardTitle>Gelen Talepler</CardTitle><CardDescription>Bu siteye gelen talepler; 3 saniyede bir ve pencere odaklandığında yenilenir.</CardDescription></div><Button variant="outline" onClick={() => void loadLeads()}><RefreshCw className="mr-2 h-4 w-4" />Yenile</Button></CardHeader><CardContent>{siteLeads.length ? <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead className="border-y text-xs text-[#7a857e]"><tr><th className="py-4">Ad</th><th>Telefon</th><th>Mesaj</th><th>Tarih</th></tr></thead><tbody>{siteLeads.map((lead) => <tr key={lead.id} className="border-b"><td className="py-5 font-semibold">{lead.name}</td><td><a href={`tel:${lead.phone}`}>{lead.phone}</a></td><td className="max-w-sm text-sm">{lead.message || "—"}</td><td className="text-xs text-[#7a857e]">{formatDateTR(lead.created_at)}</td></tr>)}</tbody></table></div> : <p className="p-5 text-sm text-[#69756e]">Henüz talep yok.</p>}</CardContent></Card> : null}

      {activeSite && activeTab === "site" && siteDraft ? <div className="grid gap-6 xl:grid-cols-2"><Card className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7]"><CardHeader><CardTitle>Site ve iletişim bilgileri</CardTitle><CardDescription>Değerler sites tablosundaki theme_config içeriğine ve ilgili ana kolonlara kaydedilir.</CardDescription></CardHeader><CardContent className="space-y-4"><div><Label>İşletme adı</Label><Input value={siteDraft.business_name} onChange={(e) => setSiteDraft({ ...siteDraft, business_name: e.target.value })} /></div><div><Label>Başlık</Label><Input value={siteDraft.headline} onChange={(e) => setSiteDraft({ ...siteDraft, headline: e.target.value })} /></div><div><Label>Kısa açıklama</Label><Textarea value={siteDraft.tone} onChange={(e) => setSiteDraft({ ...siteDraft, tone: e.target.value })} /></div><div className="grid gap-4 sm:grid-cols-2"><div><Label>Telefon</Label><Input value={siteDraft.phone} onChange={(e) => setSiteDraft({ ...siteDraft, phone: e.target.value })} /></div><div><Label>E-posta</Label><Input type="email" value={siteDraft.email} onChange={(e) => setSiteDraft({ ...siteDraft, email: e.target.value })} /></div></div><div><Label>Adres</Label><Input value={siteDraft.address} onChange={(e) => setSiteDraft({ ...siteDraft, address: e.target.value })} /></div><Button onClick={saveIdentity} disabled={savingSite}>Bilgileri kaydet</Button><Button variant="outline" onClick={togglePublication} disabled={savingSite}>{activeSite.status === "published" ? "Yayından kaldır" : "Yayınla"}</Button></CardContent></Card><Card className="rounded-[2rem] border-[#173f32]/10 bg-[#fbfaf7]"><CardHeader><CardTitle>Tema ayarları</CardTitle><CardDescription>Mevcut theme_config yalnızca seçtiğiniz alanlarda PATCH edilir; Gemini yeniden çağrılmaz.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid grid-cols-2 gap-4"><div><Label>Ana renk</Label><Input type="color" className="h-12 p-1" value={siteDraft.primary_color} onChange={(e) => setSiteDraft({ ...siteDraft, primary_color: e.target.value })} /></div><div><Label>Vurgu rengi</Label><Input type="color" className="h-12 p-1" value={siteDraft.accent_color} onChange={(e) => setSiteDraft({ ...siteDraft, accent_color: e.target.value })} /></div></div><div><Label>Başlık fontu</Label><Select value={siteDraft.heading_font} onValueChange={(value) => setSiteDraft({ ...siteDraft, heading_font: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Cormorant Garamond, Georgia, serif">Cormorant Garamond</SelectItem><SelectItem value="Libre Baskerville, Georgia, serif">Libre Baskerville</SelectItem><SelectItem value="Manrope, Inter, Arial, sans-serif">Manrope</SelectItem><SelectItem value="Inter, Arial, sans-serif">Inter</SelectItem></SelectContent></Select></div><div><Label>Gövde fontu</Label><Select value={siteDraft.body_font} onValueChange={(value) => setSiteDraft({ ...siteDraft, body_font: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Inter, Arial, sans-serif">Inter</SelectItem><SelectItem value="Manrope, Inter, Arial, sans-serif">Manrope</SelectItem></SelectContent></Select></div><div className="rounded-2xl p-7 text-white" style={{ background: `linear-gradient(135deg, ${siteDraft.primary_color}, ${siteDraft.accent_color})`, fontFamily: siteDraft.body_font }}><div className="text-xs opacity-70">Canlı tema önizlemesi</div><h3 className="mt-8 text-4xl" style={{ fontFamily: siteDraft.heading_font }}>{siteDraft.headline}</h3></div><Button onClick={saveTheme} disabled={savingSite}>{savingSite ? "Kaydediliyor..." : "Temayı kaydet"}</Button></CardContent></Card></div> : null}
    </div>
  </Shell>;
}
