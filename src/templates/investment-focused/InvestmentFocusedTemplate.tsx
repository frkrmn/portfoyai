...
function InvestmentCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <article className="group border border-[var(--if-line)] bg-[var(--if-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[var(--if-soft)]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-[1.025]" />{listing.rental_yield_percent != null ? <span className="absolute left-4 top-4 bg-[var(--if-accent)] px-3 py-2 text-[10px] font-bold text-[var(--if-on-primary)]">{c.rentalYieldLabel} {formatYield(listing.rental_yield_percent)}</span> : null}</Link><div className="p-5"><div className="flex items-center gap-1.5 text-xs opacity-55"><MapPin className="h-3.5 w-3.5" />{formatListingLocation(listing)}</div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-2 font-[family-name:var(--if-heading)] text-xl font-extrabold leading-tight">{listing.title}</h3></Link><div className="mt-5 grid grid-cols-2 gap-3"><Metric label={c.pricePerM2Label} value={formatListingPrice(listing, pricePerM2(listing))} /><Metric label={c.rentalYieldLabel} value={formatYield(listing.rental_yield_percent)} /></div>{listing.roi_notes ? <p className="mt-4 border-l-2 border-[var(--if-accent)] pl-3 text-xs leading-5 opacity-65">{listing.roi_notes}</p> : null}<div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--if-line)] pt-4"><div><strong className="block text-lg">{formatListingPrice(listing)}</strong><div className="mt-2"><Specs config={config} listing={listing} /></div></div><ArrowRight className="h-4 w-4 text-[var(--if-accent)]" /></div></div></article>;
}
