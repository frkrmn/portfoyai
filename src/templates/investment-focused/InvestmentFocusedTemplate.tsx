...
const formatMoney = (value: number, currency: Listing["currency"] = "TRY") => new Intl.NumberFormat("tr-TR", { style: "currency", currency, maximumFractionDigits: 0 }).format(Number(value));
...
function InvestmentCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <article className="group border border-[var(--if-line)] bg-[var(--if-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[var(--if-soft)]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-[1.025]" />{listing.rental_yield_percent != null ? <span className="absolute left-4 top-4 bg-[var(--if-accent)] px-3 py-2 text-[10px] font-bold text-[var(--if-on-primary)]">{c.rentalYieldLabel} {formatYield(listing.rental_yield_percent)}</span> : null}</Link><div className="p-5"><div className="flex items-center gap-1.5 text-xs opacity-55"><MapPin className="h-3.5 w-3.5" />{listing.address || listing.district}</div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-2 font-[family-name:var(--if-heading)] text-xl font-extrabold leading-tight">{listing.title}</h3></Link><div className="mt-5 grid grid-cols-2 gap-3"><Metric label={c.pricePerM2Label} value={formatMoney(pricePerM2(listing), listing.currency)} /><Metric label={c.rentalYieldLabel} value={formatYield(listing.rental_yield_percent)} /></div>{listing.roi_notes ? <p className="mt-4 border-l-2 border-[var(--if-accent)] pl-3 text-xs leading-5 opacity-65">{listing.roi_notes}</p> : null}<div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--if-line)] pt-4"><div><strong className="block text-lg">{formatMoney(listing.price, listing.currency)}</strong><div className="mt-2"><Specs config={config} listing={listing} /></div></div><ArrowRight className="h-4 w-4 text-[var(--if-accent)]" /></div></div></article>;
}

function ComparisonTable({ config, listings }: { config: TemplateConfig; listings: Listing[] }) {
  const c = config.content;
  return <div className="overflow-x-auto border border-[var(--if-line)]"><table className="w-full min-w-[900px] border-collapse text-left text-sm"><thead className="bg-[var(--if-soft)] text-[10px] uppercase tracking-[0.12em]"><tr><th className="px-5 py-4">{c.listingsTitle}</th><th className="px-5 py-4">{c.priceRangeLabel}</th><th className="px-5 py-4">{c.areaLabel}</th><th className="px-5 py-4">{c.pricePerM2Label}</th><th className="px-5 py-4">{c.rentalYieldLabel}</th><th className="px-5 py-4">{c.detailsLabel}</th></tr></thead><tbody className="divide-y divide-[var(--if-line)]">{listings.map((listing) => <tr key={listing.id} className="hover:bg-[var(--if-soft)]"><td className="px-5 py-5"><div className="font-semibold">{listing.title}</div><div className="mt-1 text-xs opacity-50">{listing.address || listing.district}</div></td><td className="whitespace-nowrap px-5 py-5 font-semibold">{formatMoney(listing.price, listing.currency)}</td><td className="px-5 py-5">{listing.m2} m²</td><td className="whitespace-nowrap px-5 py-5">{formatMoney(pricePerM2(listing), listing.currency)}</td><td className="px-5 py-5"><span className="bg-[var(--if-metric)] px-3 py-2 font-bold text-[var(--if-accent)]">{formatYield(listing.rental_yield_percent)}</span></td><td className="px-5 py-5"><Link className="inline-flex items-center gap-2 font-bold text-[var(--if-accent)]" to={`/site/${config.slug}/listings/${listing.id}`}>{c.detailsLabel}<ArrowRight className="h-3.5 w-3.5" /></Link></td></tr>)}</tbody></table></div>;
}
...
export function InvestmentFocusedListings({ config }: SiteTemplateProps) {
  ...
  const filtered = useMemo(() => config.listings.filter((listing) => {
    ...
    if (location && !normalize(`${listing.district} ${listing.address || ""}`).includes(normalize(location))) return false;
  ...
  return <div className="min-h-screen" style={investmentStyle(config)}><Header config={config} /><main><section className="border-b border-[var(--if-line)] bg-[var(--if-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1400px]"><div className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--if-accent)]">{c.featuredEyebrow}</div><h1 className="mt-4 font-[family-name:var(--if-heading)] text-5xl font-extrabold tracking-[-0.04em]">{c.listingsTitle}</h1><p className="mt-4 max-w-2xl text-sm leading-7 opacity-62">{c.listingsDescription}</p></div></section><section className="mx-auto max-w-[1400px] px-5 py-10 sm:px-8 lg:px-10"><div className="grid gap-3 border border-[var(--if-line)] p-4 md:grid-cols-[1fr_1fr_1fr_auto]"><label className="relative"><Search className="absolute left-4 top-4 h-4 w-4 opacity-40" /><input className={`${field} pl-11`} placeholder={c.searchLabel} value={query} onChange={(event) => setQuery(event.target.value)} /></label><input className={field} placeholder={c.locationLabel} value={location} onChange={(event) => setLocation(event.target.value)} /><select className={field} value={type} onChange={(event) => setType(event.target.value)}><option value="all">{c.allLabel}</option><option value="sale">{c.saleLabel}</option><option value="rent">{c.rentLabel}</option></select><div className="flex border border-[var(--if-line)] p-1"><button aria-pressed={view === "cards"} onClick={() => setView("cards")} className={`flex items-center gap-2 px-4 text-xs font-bold ${view === "cards" ? "bg-[var(--if-primary)] text-[var(--if-on-primary)]" : ""}`}><Grid2X2 className="h-4 w-4" />{c.cardViewLabel}</button><button aria-pressed={view === "comparison"} onClick={() => setView("comparison")} className={`flex items-center gap-2 px-4 text-xs font-bold ${view === "comparison" ? "bg-[var(--if-primary)] text-[var(--if-on-primary)]" : ""}`}><List className="h-4 w-4" />{c.comparisonViewLabel}</button></div></div>{filtered.length ? view === "cards" ? <div data-view="cards" className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((listing) => <InvestmentCard key={listing.id} config={config} listing={listing} />)}</div> : <div data-view="comparison" className="mt-10"><ComparisonTable config={config} listings={filtered} /></div> : <p className="mt-10 border border-[var(--if-line)] p-8 text-sm opacity-60">{c.emptyListings}</p>}</section></main><Footer config={config} /></div>;
}
