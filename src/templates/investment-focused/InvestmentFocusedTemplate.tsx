...
export function InvestmentFocusedListings({ config }: SiteTemplateProps) {
  ...
  return <div {...fineTuneAttributes(config)} style={investmentStyle(config)}><Header config={config} /><main><section className="border-b border-[var(--if-line)] bg-[var(--if-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1400px]"><div className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--if-accent)]">{c.featuredEyebrow}</div><h1 className="mt-4 font-[family-name:var(--if-heading)] text-5xl font-extrabold tracking-[-0.04em]">{c.listingsTitle}</h1><p className="mt-4 max-w-2xl text-sm leading-7 opacity-62">{c.listingsDescription}</p></div></section><section className="mx-auto max-w-[1400px] px-5 py-10 sm:px-8 lg:px-10"><div className="grid gap-3 border border-[var(--if-line)] p-4 md:grid-cols-[1fr_1fr_1fr_auto]"><label className="relative"><Search className="absolute left-4 top-4 h-4 w-4 opacity-40" /><input className={`${field} pl-11`} placeholder={c.searchLabel} value={query} onChange={(event) => setQuery(event.target.value)} /></label><input className={field} placeholder={c.locationLabel} value={location} onChange={(event) => setLocation(event.target.value)} /><select className={field} value={type} onChange={(event) => setType(event.target.value)}><option value="all">{c.allLabel}</option><option value="sale">{c.saleLabel}</option><option value="rent">{c.rentLabel}</option></select><div className="flex border border-[var(--if-line)] p-1"><button data-site-button aria-pressed={view === "cards"} onClick={() => setView("cards")} className={`flex items-center gap-2 px-4 text-xs font-bold ${view === "cards" ? "bg-[var(--if-primary)] text-[var(--if-on-primary)]" : ""}`}><Grid2X2 className="h-4 w-4" />{c.cardViewLabel}</button><button data-site-button aria-pressed={view === "comparison"} onClick={() => setView("comparison")} className={`flex items-center gap-2 px-4 text-xs font-bold ${view === "comparison" ? "bg-[var(--if-primary)] text-[var(--if-on-primary)]" : ""}`}><List className="h-4 w-4" />{c.comparisonViewLabel}</button></div></div>{filtered.length ? view === "cards" ? <div data-view="cards" className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((listing) => <InvestmentCard key={listing.id} config={config} listing={listing} />)}</div> : <div data-view="comparison" className="mt-10"><ComparisonTable config={config} listings={filtered} /></div> : <p className="mt-10 border border-[var(--if-line)] p-8 text-sm opacity-60">{c.emptyListings}</p>}</section><ClosedListingsGroups config={config} renderListing={(listing) => <InvestmentCard config={config} listing={listing} />}/></main><Footer config={config} /></div>;
}
