...
function LuxuryListingCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  ...
  return <article className="group"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[color:color-mix(in_srgb,var(--lux-ink)_8%,transparent)]">{image ? <img src={image} alt={listing.title} className="aspect-[4/3] w-full object-cover transition duration-700 group-hover:scale-105" /> : <div className="aspect-[4/3]" />}<span className="absolute left-4 top-4 bg-[var(--lux-accent)] px-3 py-2 text-[9px] font-bold uppercase tracking-[0.14em] text-[var(--lux-ink)]">{listing.listing_type === "sale" ? c.saleLabel : c.rentLabel}</span><PropertyTaxonomyBadge config={config} listing={listing} className="absolute bottom-4 left-4 bg-[var(--lux-dark)] px-3 py-2 text-[9px] uppercase tracking-[0.12em] text-[var(--lux-text)]" /></Link><div className="pt-5"><div className="text-[10px] uppercase tracking-[0.14em] opacity-45">{formatListingLocation(listing)}</div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h2 className="mt-2 font-[family-name:var(--lux-heading)] text-3xl font-semibold leading-tight">{listing.title}</h2></Link><div className="mt-5 flex flex-wrap items-center gap-4 border-t border-[color:color-mix(in_srgb,var(--lux-ink)_16%,transparent)] pt-4 text-xs"><span className="flex items-center gap-1.5"><Maximize2 className="h-3.5 w-3.5" />{listing.m2} m²</span><span className="flex items-center gap-1.5"><BedDouble className="h-3.5 w-3.5" />{listing.room_count}</span><span className="flex items-center gap-1.5"><Bath className="h-3.5 w-3.5" />{listing.bathroom_count || 1}</span><strong className="ml-auto whitespace-nowrap">{formatListingPrice(listing)}</strong></div></div></article>;
}
...
export function BoldLuxuryListings({ config }: SiteTemplateProps) {
  ...
  const filtered = useMemo(() => config.listings.filter((listing) => {
    ...
    if (location && !`${formatListingLocation(listing)} ${listing.address || ""}`.toLocaleLowerCase("tr-TR").includes(location.toLocaleLowerCase("tr-TR"))) return false;
