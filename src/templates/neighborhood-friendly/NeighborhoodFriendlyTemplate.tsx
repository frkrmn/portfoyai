...
const locationText = (listing: Listing) => `${listing.district} ${listing.address || ""} ${listing.title}`;
...
const formatPrice = (listing: Listing) => new Intl.NumberFormat("tr-TR", { style: "currency", currency: listing.currency || "TRY", maximumFractionDigits: 0 }).format(Number(listing.price));
...
export function NeighborhoodFriendlyListings({ config }: SiteTemplateProps) {
  ...
  return <div className="min-h-screen" style={friendlyStyle(config)}><Header config={config} /><main><section className="bg-[var(--nf-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1360px]"><div className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--nf-accent)]">{location || c.eyebrow}</div><h1 className="mt-4 font-[family-name:var(--nf-heading)] text-5xl font-extrabold tracking-[-0.04em]">{c.listingsTitle}</h1><p className="mt-4 max-w-2xl text-sm leading-7 opacity-65">{c.listingsDescription}</p></div></section><section className="mx-auto max-w-[1360px] px-5 py-12 sm:px-8 lg:px-10"><div className="grid gap-3 rounded-[2rem] border border-[var(--nf-line)] p-4 sm:grid-cols-3"><input className={field} placeholder={c.searchLabel} value={query} onChange={(event) => setQuery(event.target.value)} /><select className={field} value={location} onChange={(event) => setLocation(event.target.value)}><option value="">{c.locationLabel}</option>{c.neighborhoods.map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</select><select className={field} value={type} onChange={(event) => setType(event.target.value)}><option value="all">{c.allLabel}</option><option value="sale">{c.saleLabel}</option><option value="rent">{c.rentLabel}</option></select></div>{filtered.length ? <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((listing) => <FriendlyCard key={listing.id} config={config} listing={listing} />)}</div> : <p className="mt-10 rounded-[2rem] border border-[var(--nf-line)] p-8 text-sm opacity-60">{c.emptyListings}</p>}</section></main><Footer config={config} /></div>;
}
...
export function NeighborhoodFriendlyDetail({ config }: SiteTemplateProps) {
  ...
  return <div className="min-h-screen" style={friendlyStyle(config)}><Header config={config} /><main><section className="mx-auto max-w-[1360px] px-5 py-10 sm:px-8 lg:px-10"><Link to={`/site/${config.slug}/listings`} className="inline-flex items-center gap-2 text-xs font-bold"><ArrowLeft className="h-4 w-4" />{c.backLabel}</Link><div className="mt-8 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="flex items-center gap-2 text-sm text-[var(--nf-accent)]"><MapPin className="h-4 w-4" />{listing.address || listing.district}</div><h1 className="mt-3 max-w-4xl font-[family-name:var(--nf-heading)] text-5xl font-extrabold leading-[0.96] tracking-[-0.04em]">{listing.title}</h1></div><strong className="text-3xl">{formatPrice(listing)}</strong></div><div className="mt-9 grid gap-4 lg:grid-cols-[1.4fr_.6fr]"><div className="overflow-hidden rounded-[2.75rem] bg-[var(--nf-soft)]">{getListingImage(listing) ? <img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/10] h-full w-full object-cover" /> : null}</div><div className="grid grid-cols-2 gap-4 lg:grid-cols-1">{[1, 2].map((index) => getListingImage(listing, index) ? <img key={index} src={getListingImage(listing, index)} alt={listing.title} className="h-full min-h-[190px] w-full rounded-[2rem] object-cover" /> : <div key={index} className="min-h-[190px] rounded-[2rem] bg-[var(--nf-soft)]" />)}</div></div><div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-[2rem] bg-[var(--nf-soft)] p-5"><span className="rounded-full bg-[var(--nf-accent)] px-4 py-2 text-xs font-bold text-[var(--nf-on-primary)]">{listing.listing_type === "sale" ? c.saleLabel : c.rentLabel}</span><Specs config={config} listing={listing} /></div></section><section className="border-t border-[var(--nf-line)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1360px] gap-12 lg:grid-cols-[1fr_420px]"><div><h2 className="font-[family-name:var(--nf-heading)] text-3xl font-extrabold">{c.listingAboutLabel}</h2><p className="mt-5 text-base leading-8 opacity-68">{listing.description}</p><h2 className="mt-12 border-t border-[var(--nf-line)] pt-10 font-[family-name:var(--nf-heading)] text-3xl font-extrabold">{c.listingFeaturesLabel}</h2><div className="mt-6 grid gap-3 sm:grid-cols-2">{listing.features.map((feature) => <div key={feature} className="rounded-2xl bg-[var(--nf-soft)] p-4 text-sm">{feature}</div>)}</div></div><aside id="iletisim" className="h-fit rounded-[2.5rem] bg-[var(--nf-soft)] p-7 lg:sticky lg:top-6"><h2 className="font-[family-name:var(--nf-heading)] text-3xl font-extrabold">{c.tourTitle}</h2><p className="mt-3 text-sm leading-6 opacity-65">{c.tourDescription}</p><div className="mt-6"><LeadForm config={config} listing={listing} /></div></aside></div></section></main><Footer config={config} /></div>;
}
