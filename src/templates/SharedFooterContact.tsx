...
export function SharedFooterContact({ config }: SiteTemplateProps) {
  const { address, email, emailLabel, phone, phoneLabel } = config.content;
  ...
  return (
    <address data-footer-contact className="mx-auto mt-8 grid max-w-[1400px] gap-6 border-t border-current/15 pt-7 text-sm not-italic sm:grid-cols-3">
      <div>
        ...
        {address ? <a href={mapsHref} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1.5 border-b border-current/30 pb-0.5 text-xs font-semibold">{mapLabel}<ExternalLink className="h-3 w-3" /></a> : null}
      </div>
      <div>
