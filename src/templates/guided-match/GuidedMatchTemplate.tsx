...
function Footer({ config }: SiteTemplateProps) {
  const c = config.content;
  return <><SharedTeamSection config={config} /><footer id="iletisim" className="border-t border-[var(--gm-line)] bg-[var(--gm-soft)] px-5 py-12 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1360px] gap-8 md:grid-cols-[1fr_auto]"><div><div className="font-[family-name:var(--gm-heading)] text-3xl italic">{c.agentName}</div><p className="mt-3 max-w-lg text-sm leading-6 opacity-65">{c.tagline}</p><div className="mt-5 text-xs opacity-45">Fastate AI ile hazırlandı</div></div><address className="space-y-2 text-sm not-italic opacity-70"><div>{c.phone}</div><div>{c.email}</div><div>{c.address}</div></address></div><SharedTeamFooterLink config={config} /></footer></>;
}
...
function ListingCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <article className="group overflow-hidden rounded-[1.5rem] border border-[var(--gm-line)] bg-[var(--gm-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-[1.035]" /><span className="absolute left-4 top-4 rounded-full bg-[var(--gm-bg)] px-3 py-1.5 text-[10px] font-bold">{listing.listing_type === "sale" ? c.saleLabel : c.rentLabel}</span><PropertyTaxonomyBadge config={config} listing={listing} className="absolute bottom-4 left-4 rounded-full bg-[var(--gm-bg)] px-3 py-1.5 text-[10px] font-bold text-[var(--gm-accent)]" /></Link><div className="p-5"><div className="flex items-center gap-1.5 text-xs text-[var(--gm-accent)]"><MapPin className="h-3.5 w-3.5" />{formatListingLocation(listing)}</div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-2 line-clamp-2 font-[family-name:var(--gm-heading)] text-2xl leading-tight">{listing.title}</h3></Link><div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--gm-line)] pt-4"><Specs config={config} listing={listing} /><strong className="shrink-0 text-lg text-[var(--gm-accent)]">{formatListingPrice(listing)}</strong></div></div></article>;
}
