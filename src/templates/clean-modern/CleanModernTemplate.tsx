...
export const imageSchema = imageSlots([
  ...
  { key: "media.testimonialImage", label: "Referans Bölümü Görseli", type: "single", recommendedSize: "1600x900" },
]);
...
export function CleanModernHome({ config }: SiteTemplateProps) {
  ...
  const heroImage = getHeroImage(config.siteId, String(config.media.heroImage || config.content.heroImage || (featured?.media?.length ? getListingImage(featured) : undefined)));
  const testimonialListing = config.listings[2] || featured;
  const testimonialImage = String(config.media.testimonialImage || (testimonialListing ? getListingImage(testimonialListing, 1) : getHeroImage(config.siteId, config.content.heroImage)));
