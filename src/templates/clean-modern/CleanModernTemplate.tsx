...
const propertyTypeFor = (config: TemplateConfig, listing: Listing) => {
  if (listing.category === "house") return config.content.houseLabel;
  if (listing.category === "duplex") return config.content.duplexLabel;
  return config.content.apartmentLabel;
};
...
function ListingCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  ...
  return <article className="group border border-[var(--cm-line)] bg-[var(--cm-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[var(--cm-soft)]">{image ? <img src={image} alt={listing.title} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-[1.03]" /> : <div className="aspect-[4/3]" />}<span className="absolute left-3 top-3 bg-[var(--cm-primary)] px-2.5 py-1.5 text-[9px] font-bold uppercase tracking-wider text-[var(--cm-on-primary)]">{listing.listing_type === "sale" ? c.saleLabel : c.rentLabel}</span></Link><div className="p-5"><div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--cm-accent)]">{propertyTypeFor(config, listing)}</div><div className="mt-2 flex items-start gap-1.5 text-xs opacity-55"><MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0" />{listing.address || listing.district}</div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-2 line-clamp-1 font-[family-name:var(--cm-heading)] text-lg font-bold">{listing.title}</h3></Link><strong className="mt-4 block text-xl text-[var(--cm-accent)]">{formatPrice(listing)}</strong><div className="mt-4 border-t border-[var(--cm-line)] pt-4"><Specs config={config} listing={listing} compact /></div></div></article>;
}
...
function SearchPanel({ config, initialType = "all" }: SiteTemplateProps & { initialType?: string }) {
  ...
  const [filters, setFilters] = useState<FilterState>({ query: "", location: "", propertyType: "", rooms: "", maxPrice: "" });
  ...
  return <form onSubmit={submit} className="border border-[var(--cm-line)] bg-[var(--cm-bg)] p-5 shadow-[0_18px_50px_color-mix(in_srgb,var(--cm-text)_10%,transparent)] sm:p-7"><div className="flex items-end justify-between gap-6"><div><h2 className="font-[family-name:var(--cm-heading)] text-2xl font-extrabold">{c.findHomeTitle}</h2><p className="mt-1 text-xs opacity-55">{c.findHomeDescription}</p></div><div className="flex border border-[var(--cm-line)] p-1 text-xs font-bold"><button type="button" onClick={() => setType("sale")} className={`px-4 py-2 ${type === "sale" ? "bg-[var(--cm-primary)] text-[var(--cm-on-primary)]" : ""}`}>{c.saleLabel}</button><button type="button" onClick={() => setType("rent")} className={`px-4 py-2 ${type === "rent" ? "bg-[var(--cm-primary)] text-[var(--cm-on-primary)]" : ""}`}>{c.rentLabel}</button></div></div><div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-[1.1fr_1fr_1fr_.8fr_1fr_auto]"><input className={field} placeholder={c.keywordLabel} value={filters.query} onChange={(event) => set("query", event.target.value)} /><input className={field} placeholder={c.locationLabel} value={filters.location} onChange={(event) => set("location", event.target.value)} /><select aria-label={c.propertyTypeLabel} className={field} value={filters.propertyType} onChange={(event) => set("propertyType", event.target.value)}><option value="">{c.propertyTypeLabel}</option><option value="apartment">{c.apartmentLabel}</option><option value="house">{c.houseLabel}</option><option value="duplex">{c.duplexLabel}</option></select><input className={field} placeholder={c.roomLabel} value={filters.rooms} onChange={(event) => set("rooms", event.target.value)} /><input type="number" min="0" className={field} placeholder={c.priceRangeLabel} value={filters.maxPrice} onChange={(event) => set("maxPrice", event.target.value)} /><button className="flex h-11 items-center justify-center gap-2 bg-[var(--cm-primary)] px-5 text-xs font-bold text-[var(--cm-on-primary)]"><Search className="h-4 w-4" />{c.searchLabel}</button></div></form>;
}
...
export function CleanModernHome({ config }: SiteTemplateProps) {
  ...
  return <div className="min-h-screen" style={cleanStyle(config)}><Header config={config} /><main><section className="relative min-h-[650px] bg-[var(--cm-soft)]">{heroImage ? <img src={heroImage} alt={featured?.title || c.headline} className="absolute inset-0 h-full w-full object-cover" /> : null}<div className="absolute inset-0 bg-[linear-gradient(90deg,color-mix(in_srgb,var(--cm-primary)_32%,transparent),transparent)]" /><div className="relative mx-auto flex min-h-[650px] max-w-[1380px] items-end px-5 pb-14 sm:px-8 lg:px-10">{featured ? <article className="w-full max-w-[500px] border-t-4 border-[var(--cm-accent)] bg-[var(--cm-bg)] p-7 shadow-[0_22px_65px_color-mix(in_srgb,var(--cm-text)_20%,transparent)] sm:p-9"><div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--cm-accent)]">{featured.listing_type === "sale" ? c.saleLabel : c.rentLabel}</div><strong className="mt-3 block text-3xl text-[var(--cm-accent)]">{formatPrice(featured)}</strong><div className="mt-3 flex items-center gap-1.5 text-xs opacity-55"><MapPin className="h-3.5 w-3.5" />{featured.address || featured.district}</div><h1 className="mt-3 font-[family-name:var(--cm-heading)] text-2xl font-extrabold">{featured.title}</h1><p className="mt-3 line-clamp-2 text-sm leading-6 opacity-62">{featured.description}</p><div className="mt-5 border-t border-[var(--cm-line)] pt-5"><Specs config={config} listing={featured} /></div><Link to={`/site/${config.slug}/listings/${featured.id}`} className="mt-6 inline-flex items-center gap-2 bg-[var(--cm-primary)] px-5 py-3 text-xs font-bold text-[var(--cm-on-primary)]">{c.detailsLabel}<ArrowRight className="h-4 w-4" /></Link></article> : <div className="max-w-2xl bg-[var(--cm-bg)] p-9"><div className="text-xs font-bold uppercase tracking-wider text-[var(--cm-accent)]">{c.eyebrow}</div><h1 className="mt-4 font-[family-name:var(--cm-heading)] text-5xl font-extrabold">{c.headline}</h1><p className="mt-4 text-sm leading-7 opacity-60">{c.bio}</p></div>}</div></section><section className="relative z-10 mx-auto -mt-1 max-w-[1380px] px-5 sm:px-8 lg:-mt-9 lg:px-10"><SearchPanel config={config} initialType="sale" /></section><ListingSection config={config} type="rent" title={c.rentSectionTitle} /><div className="border-y border-[var(--cm-line)] bg-[var(--cm-soft)]"><ListingSection config={config} type="sale" title={c.saleSectionTitle} /></div>{config.layout.showTestimonial ? <section id="hakkimizda" className="relative min-h-[460px] overflow-hidden">{testimonialImage ? <img src={testimonialImage} alt={c.testimonialAuthor} className="absolute inset-0 h-full w-full object-cover" /> : null}<div className="absolute inset-0 bg-[color:color-mix(in_srgb,var(--cm-primary)_72%,transparent)]" /><div className="relative mx-auto flex min-h-[460px] max-w-[1380px] items-center px-5 sm:px-8 lg:px-10"><blockquote className="max-w-2xl bg-[var(--cm-bg)] p-8 sm:p-12"><p className="font-[family-name:var(--cm-heading)] text-3xl font-bold leading-tight">“{c.testimonialQuote}”</p><footer className="mt-7 border-t border-[var(--cm-line)] pt-5 text-sm"><strong>{c.testimonialAuthor}</strong><span className="ml-2 opacity-55">{c.testimonialRole}</span></footer></blockquote></div></section> : null}</main><Footer config={config} /></div>;
}

export function CleanModernListings({ config }: SiteTemplateProps) {
  const c = config.content;
  const [params] = useSearchParams();
  const [filters, setFilters] = useState<FilterState>({ query: params.get("query") || "", location: params.get("location") || "", propertyType: params.get("propertyType") || "", rooms: params.get("rooms") || "", maxPrice: params.get("maxPrice") || "" });
  ...
  const filtered = useMemo(() => config.listings.filter((listing) => {
    ...
    if (filters.location && !normalize(`${listing.district} ${listing.address || ""}`).includes(normalize(filters.location))) return false;
    if (filters.propertyType && !normalize(listing.category || "").includes(normalize(filters.propertyType))) return false;
  ...
  const field = "h-11 w-full border border-[var(--cm-line)] bg-[var(--cm-bg)] px-3 text-xs outline-none focus:border-[var(--cm-accent)]";
  return <div className="min-h-screen" style={cleanStyle(config)}><Header config={config} /><main><section className="border-b border-[var(--cm-line)] bg-[var(--cm-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto max-w-[1380px]"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--cm-accent)]">{c.featuredEyebrow}</div><h1 className="mt-3 font-[family-name:var(--cm-heading)] text-5xl font-extrabold tracking-[-0.035em]">{c.listingsTitle}</h1><p className="mt-4 max-w-2xl text-sm leading-7 opacity-60">{c.listingsDescription}</p></div></section><section className="mx-auto max-w-[1380px] px-5 py-10 sm:px-8 lg:px-10"><div className="grid gap-3 border border-[var(--cm-line)] p-5 sm:grid-cols-2 lg:grid-cols-6"><input className={field} placeholder={c.keywordLabel} value={filters.query} onChange={(event) => set("query", event.target.value)} /><input className={field} placeholder={c.locationLabel} value={filters.location} onChange={(event) => set("location", event.target.value)} /><select aria-label={c.propertyTypeLabel} className={field} value={filters.propertyType} onChange={(event) => set("propertyType", event.target.value)}><option value="">{c.propertyTypeLabel}</option><option value="apartment">{c.apartmentLabel}</option><option value="house">{c.houseLabel}</option><option value="duplex">{c.duplexLabel}</option></select><input className={field} placeholder={c.roomLabel} value={filters.rooms} onChange={(event) => set("rooms", event.target.value)} /><input type="number" className={field} placeholder={c.priceRangeLabel} value={filters.maxPrice} onChange={(event) => set("maxPrice", event.target.value)} /><select className={field} value={type} onChange={(event) => setType(event.target.value)}><option value="all">{c.allLabel}</option><option value="sale">{c.saleLabel}</option><option value="rent">{c.rentLabel}</option></select></div>{filtered.length ? <div className="mt-9 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filtered.map((listing) => <ListingCard key={listing.id} config={config} listing={listing} />)}</div> : <p className="mt-9 border border-[var(--cm-line)] p-7 text-sm opacity-55">{c.emptyListings}</p>}</section></main><Footer config={config} /></div>;
}
...
export function CleanModernDetail({ config }: SiteTemplateProps) {
  ...
  return <div className="min-h-screen" style={cleanStyle(config)}><Header config={config} /><main><section className="mx-auto max-w-[1380px] px-5 py-10 sm:px-8 lg:px-10"><Link to={`/site/${config.slug}/listings`} className="inline-flex items-center gap-2 text-xs font-bold"><ArrowLeft className="h-4 w-4" />{c.backLabel}</Link><div className="mt-7 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="flex items-center gap-1.5 text-xs text-[var(--cm-accent)]"><MapPin className="h-4 w-4" />{listing.address || listing.district}</div><h1 className="mt-3 max-w-4xl font-[family-name:var(--cm-heading)] text-4xl font-extrabold tracking-[-0.03em] sm:text-5xl">{listing.title}</h1></div><strong className="text-3xl text-[var(--cm-accent)]">{formatPrice(listing)}</strong></div><div className="mt-9 grid gap-4 lg:grid-cols-[1.5fr_.5fr]"><div className="bg-[var(--cm-soft)]">{getListingImage(listing) ? <img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/9] h-full w-full object-cover" /> : null}</div><div className="grid grid-cols-2 gap-4 lg:grid-cols-1">{[1, 2].map((index) => getListingImage(listing, index) ? <img key={index} src={getListingImage(listing, index)} alt={listing.title} className="h-full min-h-[180px] w-full object-cover" /> : <div key={index} className="min-h-[180px] bg-[var(--cm-soft)]" />)}</div></div><div className="mt-4 flex flex-wrap items-center justify-between gap-5 border border-[var(--cm-line)] p-5"><span className="bg-[var(--cm-primary)] px-3 py-2 text-[10px] font-bold uppercase text-[var(--cm-on-primary)]">{listing.listing_type === "sale" ? c.saleLabel : c.rentLabel}</span><Specs config={config} listing={listing} /></div></section><section className="border-t border-[var(--cm-line)] bg-[var(--cm-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1380px] gap-12 lg:grid-cols-[1fr_420px]"><div><h2 className="font-[family-name:var(--cm-heading)] text-3xl font-extrabold">{c.listingAboutLabel}</h2><p className="mt-5 text-sm leading-8 opacity-68">{listing.description}</p><h2 className="mt-12 border-t border-[var(--cm-line)] pt-10 font-[family-name:var(--cm-heading)] text-3xl font-extrabold">{c.listingFeaturesLabel}</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{listing.features.map((feature) => <div key={feature} className="border border-[var(--cm-line)] bg-[var(--cm-bg)] p-4 text-sm">{feature}</div>)}</div></div><aside id="iletisim" className="h-fit border-t-4 border-[var(--cm-accent)] bg-[var(--cm-bg)] p-7 shadow-[0_18px_55px_color-mix(in_srgb,var(--cm-text)_10%,transparent)] lg:sticky lg:top-5"><h2 className="font-[family-name:var(--cm-heading)] text-2xl font-extrabold">{c.tourTitle}</h2><p className="mt-3 text-sm leading-6 opacity-60">{c.tourDescription}</p><div className="mt-6"><LeadForm config={config} listing={listing} /></div></aside></div></section></main><Footer config={config} /></div>;
}
