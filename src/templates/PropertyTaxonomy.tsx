...
export type PropertyTaxonomyFilter = "all" | "konut" | "konut:daire" | "konut:mustakil_ev" | "konut:villa" | "konut:rezidans" | "arsa" | "isyeri";

export const propertyCategoryFor = (listing: Listing) => listing.property_category || "konut";
export const propertySubtypeFor = (listing: Listing) => propertyCategoryFor(listing) === "konut" ? (listing.property_subtype || "daire") : null;

export function propertyTaxonomyLabel(config: TemplateConfig, listing: Listing) {
  const category = propertyCategoryFor(listing);
  if (category === "arsa") return config.content.landLabel;
