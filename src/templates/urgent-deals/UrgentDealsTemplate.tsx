...
function DenseDeal({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <Link to={`/site/${config.slug}/listings/${listing.id}`} className="group grid grid-cols-[116px_1fr] gap-4 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-bg)] p-3 sm:grid-cols-[150px_1fr]"><img src={getListingImage(listing)} alt={listing.title} className="h-full min-h-[112px] w-full rounded-xl object-cover" /><div className="min-w-0 py-1"><UrgencyBadges config={config} listing={listing} /><h3 className="mt-3 line-clamp-1 font-[family-name:var(--ud-heading)] text-lg font-black">{listing.title}</h3><div className="mt-2 flex items-center gap-3 text-[11px] opacity-55"><span>{formatListingLocation(listing)}</span><span>·</span><span>{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><div className="mt-3 flex items-end gap-3"><strong className="text-lg text-[var(--ud-accent)]">{formatListingPrice(listing)}</strong>{hasReduction(listing) ? <span className="text-xs line-through opacity-40">{formatListingPrice(listing, Number(listing.price_reduced_from))}</span> : null}</div></div></Link>;
}
...
export function UrgentDealsDetail({ config }: SiteTemplateProps) {
  ...
  return <div {...fineTuneAttributes(config)} style={dealStyle(config)}><Header config={config} /><main><section className="mx-auto max-w-[1380px] px-5 py-10 sm:px-8 lg:px-10"><Link to={`/site/${config.slug}/listings`} className="inline-flex items-center gap-2 text-xs font-black"><ArrowLeft className="h-4 w-4" />{c.backLabel}</Link><div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><UrgencyBadges config={config} listing={listing} large /><div className="mt-4 flex items-center gap-4 text-xs opacity-55"><span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" />{formatListingLocation(listing)}</span><span className="flex items-center gap-1.5"><Clock3 className="h-4 w-4" />{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><h1 className="mt-4 max-w-4xl font-[family-name:var(--ud-heading)] text-5xl font-black leading-[0.95] tracking-[-0.05em]">{listing.title}</h1></div><div className="lg:text-right">{hasReduction(listing) ? <div className="text-sm line-through opacity-45">{c.originalPriceLabel}: {formatListingPrice(listing, Number(listing.price_reduced_from))}</div> : null}<strong className="mt-1 block text-4xl text-[var(--ud-accent)]">{formatListingPrice(listing)}</strong></div></div><div className="mt-9 grid gap-4 lg:grid-cols-[1.45fr_.55fr]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/9] h-full w-full rounded-2xl object-cover" /><div className="grid grid-cols-2 gap-4 lg:grid-cols-1">{[1, 2].map((index) => <img key={index} src={getListingImage(listing, index)} alt={listing.title} className="h-full min-h-[180px] w-full rounded-2xl object-cover" />)}</div></div><div className="mt-5 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-soft)] p-5"><Specs config={config} listing={listing} /></div></section><section className="border-t border-[var(--ud-line)] bg-[var(--ud-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1380px] gap-12 lg:grid-cols-[1fr_420px]"><div><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.listingAboutLabel}</h2><p className="mt-5 text-base leading-8 opacity-68">{listing.description}</p><section className="mt-10 border-t border-[var(--ud-line)] pt-10"><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.listingFeaturesLabel}</h2><div className="mt-6 grid gap-3 sm:grid-cols-2">{listing.features.map((feature) => <div key={feature} className="rounded-xl border border-[var(--ud-line)] bg-[var(--ud-bg)] p-4 text-sm">{feature}</div>)}</div></section></div><aside id="iletisim" className="h-fit rounded-2xl border-t-4 border-[var(--ud-accent)] bg-[var(--ud-bg)] p-7 lg:sticky lg:top-6"><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.tourTitle}</h2><p className="mt-3 text-sm leading-7 opacity-62">{c.tourDescription}</p><div className="mt-6"><LeadForm config={config} listing={listing} /></div></aside></div></section></main><Footer config={config} /></div>;
}
