...
function DealCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <article className="group overflow-hidden rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[var(--ud-soft)]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-[1.03]" /><div className="absolute left-4 top-4"><UrgencyBadges config={config} listing={listing} /></div></Link><div className="p-5"><div className="flex items-center justify-between gap-4 text-[11px] opacity-55"><span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{formatListingLocation(listing)}</span><span className="flex shrink-0 items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-3 font-[family-name:var(--ud-heading)] text-xl font-black leading-tight tracking-[-0.025em]">{listing.title}</h3></Link><div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--ud-line)] pt-4"><div>{hasReduction(listing) ? <div className="text-xs line-through opacity-45">{c.originalPriceLabel}: {money(Number(listing.price_reduced_from), listing.currency)}</div> : null}<strong className="mt-1 block text-xl text-[var(--ud-accent)]">{money(listing.price, listing.currency)}</strong></div><Specs config={config} listing={listing} /></div></div></article>;
}

function DenseDeal({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <Link to={`/site/${config.slug}/listings/${listing.id}`} className="group grid grid-cols-[116px_1fr] gap-4 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-bg)] p-3 sm:grid-cols-[150px_1fr]"><img src={getListingImage(listing)} alt={listing.title} className="h-full min-h-[112px] w-full rounded-xl object-cover" /><div className="min-w-0 py-1"><UrgencyBadges config={config} listing={listing} /><h3 className="mt-3 line-clamp-1 font-[family-name:var(--ud-heading)] text-lg font-black">{listing.title}</h3><div className="mt-2 flex items-center gap-3 text-[11px] opacity-55"><span>{formatListingLocation(listing)}</span><span>·</span><span>{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><div className="mt-3 flex items-end gap-3"><strong className="text-lg text-[var(--ud-accent)]">{money(listing.price, listing.currency)}</strong>{hasReduction(listing) ? <span className="text-xs line-through opacity-40">{money(Number(listing.price_reduced_from), listing.currency)}</span> : null}</div></div></Link>;
}
...
export function UrgentDealsListings({ config }: SiteTemplateProps) {
  ...
  const filtered = useMemo(() => config.listings.filter((listing) => { if (type !== "all" && listing.listing_type !== type) return false; if (dealsOnly && !isDeal(listing)) return false; if (location && !normalize(`${formatListingLocation(listing)} ${listing.address || ""}`).includes(normalize(location))) return false; return !maxPrice || listing.price <= Number(maxPrice); }), [config.listings, dealsOnly, location, maxPrice, type]);
