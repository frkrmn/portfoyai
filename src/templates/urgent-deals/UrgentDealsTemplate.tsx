...
const money = (value: number, currency: Listing["currency"] = "TRY") => new Intl.NumberFormat("tr-TR", { style: "currency", currency, maximumFractionDigits: 0 }).format(Number(value));
...
function DealCard({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <article className="group overflow-hidden rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-bg)]"><Link to={`/site/${config.slug}/listings/${listing.id}`} className="relative block overflow-hidden bg-[var(--ud-soft)]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-[1.03]" /><div className="absolute left-4 top-4"><UrgencyBadges config={config} listing={listing} /></div></Link><div className="p-5"><div className="flex items-center justify-between gap-4 text-[11px] opacity-55"><span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{listing.address || listing.district}</span><span className="flex shrink-0 items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><Link to={`/site/${config.slug}/listings/${listing.id}`}><h3 className="mt-3 font-[family-name:var(--ud-heading)] text-xl font-black leading-tight tracking-[-0.025em]">{listing.title}</h3></Link><div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--ud-line)] pt-4"><div>{hasReduction(listing) ? <div className="text-xs line-through opacity-45">{c.originalPriceLabel}: {money(Number(listing.price_reduced_from), listing.currency)}</div> : null}<strong className="mt-1 block text-xl text-[var(--ud-accent)]">{money(listing.price, listing.currency)}</strong></div><Specs config={config} listing={listing} /></div></div></article>;
}

function DenseDeal({ config, listing }: { config: TemplateConfig; listing: Listing }) {
  const c = config.content;
  return <Link to={`/site/${config.slug}/listings/${listing.id}`} className="group grid grid-cols-[116px_1fr] gap-4 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-bg)] p-3 sm:grid-cols-[150px_1fr]"><img src={getListingImage(listing)} alt={listing.title} className="h-full min-h-[112px] w-full rounded-xl object-cover" /><div className="min-w-0 py-1"><UrgencyBadges config={config} listing={listing} /><h3 className="mt-3 line-clamp-1 font-[family-name:var(--ud-heading)] text-lg font-black">{listing.title}</h3><div className="mt-2 flex items-center gap-3 text-[11px] opacity-55"><span>{listing.district}</span><span>·</span><span>{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><div className="mt-3 flex items-end gap-3"><strong className="text-lg text-[var(--ud-accent)]">{money(listing.price, listing.currency)}</strong>{hasReduction(listing) ? <span className="text-xs line-through opacity-40">{money(Number(listing.price_reduced_from), listing.currency)}</span> : null}</div></div></Link>;
}
...
export function UrgentDealsListings({ config }: SiteTemplateProps) {
  ...
  const filtered = useMemo(() => config.listings.filter((listing) => { if (type !== "all" && listing.listing_type !== type) return false; if (dealsOnly && !isDeal(listing)) return false; if (location && !normalize(`${listing.district} ${listing.address || ""}`).includes(normalize(location))) return false; return !maxPrice || listing.price <= Number(maxPrice); }), [config.listings, dealsOnly, location, maxPrice, type]);
  ...
  return <div className="min-h-screen" style={dealStyle(config)}><Header config={config} /><main><section className="border-b border-[var(--ud-line)] bg-[var(--ud-primary)] px-5 py-16 text-[var(--ud-on-primary)] sm:px-8 lg:px-10"><div className="mx-auto max-w-[1380px]"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--ud-accent)]"><Zap className="h-4 w-4" />{c.eyebrow}</div><h1 className="mt-4 font-[family-name:var(--ud-heading)] text-5xl font-black tracking-[-0.05em]">{c.listingsTitle}</h1><p className="mt-4 max-w-2xl text-sm leading-7 opacity-65">{c.listingsDescription}</p></div></section><section className="mx-auto max-w-[1380px] px-5 py-10 sm:px-8 lg:px-10"><div className="grid gap-3 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-soft)] p-4 md:grid-cols-[1fr_1fr_1fr_auto]"><input className={field} placeholder={c.locationLabel} value={location} onChange={(event) => setLocation(event.target.value)} /><input type="number" className={field} placeholder={c.priceRangeLabel} value={maxPrice} onChange={(event) => setMaxPrice(event.target.value)} /><select className={field} value={type} onChange={(event) => setType(event.target.value)}><option value="all">{c.allLabel}</option><option value="sale">{c.saleLabel}</option><option value="rent">{c.rentLabel}</option></select><button aria-pressed={dealsOnly} onClick={() => setDealsOnly((value) => !value)} className={`flex h-12 items-center justify-center gap-2 rounded-xl px-5 text-xs font-black ${dealsOnly ? "bg-[var(--ud-accent)] text-[var(--ud-on-primary)]" : "border border-[var(--ud-line)] bg-[var(--ud-bg)]"}`}><Zap className="h-4 w-4" />{c.opportunityLabel}</button></div>{filtered.length ? <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((listing) => <DealCard key={listing.id} config={config} listing={listing} />)}</div> : <p className="mt-10 rounded-2xl border border-[var(--ud-line)] p-8 text-sm opacity-60">{c.emptyListings}</p>}</section></main><Footer config={config} /></div>;
}
...
export function UrgentDealsDetail({ config }: SiteTemplateProps) {
  ...
  return <div className="min-h-screen" style={dealStyle(config)}><Header config={config} /><main><section className="mx-auto max-w-[1380px] px-5 py-10 sm:px-8 lg:px-10"><Link to={`/site/${config.slug}/listings`} className="inline-flex items-center gap-2 text-xs font-black"><ArrowLeft className="h-4 w-4" />{c.backLabel}</Link><div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><UrgencyBadges config={config} listing={listing} large /><div className="mt-4 flex items-center gap-4 text-xs opacity-55"><span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" />{listing.address || listing.district}</span><span className="flex items-center gap-1.5"><Clock3 className="h-4 w-4" />{daysOnline(listing.created_at)} {c.daysOnlineLabel}</span></div><h1 className="mt-4 max-w-4xl font-[family-name:var(--ud-heading)] text-5xl font-black leading-[0.95] tracking-[-0.05em]">{listing.title}</h1></div><div className="lg:text-right">{hasReduction(listing) ? <div className="text-sm line-through opacity-45">{c.originalPriceLabel}: {money(Number(listing.price_reduced_from), listing.currency)}</div> : null}<strong className="mt-1 block text-4xl text-[var(--ud-accent)]">{money(listing.price, listing.currency)}</strong></div></div><div className="mt-9 grid gap-4 lg:grid-cols-[1.45fr_.55fr]"><img src={getListingImage(listing)} alt={listing.title} className="aspect-[16/9] h-full w-full rounded-2xl object-cover" /><div className="grid grid-cols-2 gap-4 lg:grid-cols-1">{[1, 2].map((index) => <img key={index} src={getListingImage(listing, index)} alt={listing.title} className="h-full min-h-[180px] w-full rounded-2xl object-cover" />)}</div></div><div className="mt-5 rounded-2xl border border-[var(--ud-line)] bg-[var(--ud-soft)] p-5"><Specs config={config} listing={listing} /></div></section><section className="border-t border-[var(--ud-line)] bg-[var(--ud-soft)] px-5 py-16 sm:px-8 lg:px-10"><div className="mx-auto grid max-w-[1380px] gap-12 lg:grid-cols-[1fr_420px]"><div><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.listingAboutLabel}</h2><p className="mt-5 text-base leading-8 opacity-68">{listing.description}</p><section className="mt-10 border-t border-[var(--ud-line)] pt-10"><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.listingFeaturesLabel}</h2><div className="mt-6 grid gap-3 sm:grid-cols-2">{listing.features.map((feature) => <div key={feature} className="rounded-xl border border-[var(--ud-line)] bg-[var(--ud-bg)] p-4 text-sm">{feature}</div>)}</div></section></div><aside id="iletisim" className="h-fit rounded-2xl border-t-4 border-[var(--ud-accent)] bg-[var(--ud-bg)] p-7 lg:sticky lg:top-6"><h2 className="font-[family-name:var(--ud-heading)] text-3xl font-black">{c.tourTitle}</h2><p className="mt-3 text-sm leading-7 opacity-62">{c.tourDescription}</p><div className="mt-6"><LeadForm config={config} listing={listing} /></div></aside></div></section></main><Footer config={config} /></div>;
}
